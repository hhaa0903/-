...
const C = {
  ...
  red:"#D4806A", redLight:"#FAE8E3",
};
const SERIF="'Georgia',serif";
const SANS="'Helvetica Neue',sans-serif";
const CATS=[
  {id:"food",e:"🍜",l:"餐飲",c:"#C9A89A"},
  {id:"transport",e:"🚌",l:"交通",c:"#A8B5A2"},
  {id:"hotel",e:"🏨",l:"住宿",c:"#B5A8C8"},
  {id:"shopping",e:"🛍️",l:"購物",c:"#C8B89A"},
  {id:"activity",e:"🎭",l:"體驗",c:"#9AB5B5"},
...
const lsSet=(k,v)=>localStorage.setItem(k,JSON.stringify(v));const Lbl=({ch,s})=><div style={{fontSize:10,color:C.textFaint,letterSpacing:1.8,marginBottom:7,fontWeight:600,textTransform:"uppercase",...s}}>{ch}</div>;
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(80,70,60,0.45)",backdropFilter:"blur(12px)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
      <div style={{width:"100%",maxWidth:360,background:C.card,borderRadius:28,padding:"32px 28px",boxShadow:"0 16px 48px rgba(140,110,90,0.18)",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:28,textAlign:"left"}}>
          {[{icon:"📱",title:"離線完全可用",desc:"資料存於手機本地，飛機上也能記帳"},{icon:"🔒",title:"隱私安全",desc:"不上傳任何伺服器，資料只在你的裝置"},{icon:"👥",title:"多人旅行",desc:"自動拆帳、代購管理、一鍵 LINE 帳單"}].map(f=>(
        ...
        <button onClick={onClose} style={{width:"100%",padding:"15px",borderRadius:18,border:"none",background:"linear-gradient(135deg,#C9A89A,#D4B8AB)",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:SANS,letterSpacing:0.5}}>開始使用 →</button>
...
function useStore(){
  ...
  const addExpense=(tid,e,recs=[])=>{
    setTrips(prev=>{
      const t=prev.find(x=>x.id===tid); if(!t) return prev;
      const updated={...t,expenses:[{...e,id:uid()},...(t.expenses??[])],receivables:[...recs,...(t.receivables??[])]};
      return prev.map(x=>x.id===tid?updated:x);
    });
  };
  const delExpense=(tid,eid)=>{setTrips(prev=>prev.map(t=>t.id===tid?{...t,expenses:(t.expenses??[]).filter(e=>e.id!==eid)}:t));};
  const editExpense=(tid,eid,patch,newRecs=[])=>{
    setTrips(prev=>prev.map(t=>{
      if(t.id!==tid) return t;
      const filteredRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);
      const recsWithId=newRecs.map(r=>({...r,expenseId:eid}));
      return{...t,expenses:(t.expenses??[]).map(e=>e.id===eid?{...e,...patch,id:eid}:e),receivables:[...recsWithId,...filteredRecs]};
    }));
  };
  ...
  const restoreAll=(data)=>{
    ...
    if(data.trips?.length>0) setActiveId(data.trips[0].id);
  };
  return{trips,activeId,setActiveId,active,banks,setBanks,cards,setCards,friends,setFriends,
    ...
    addExpense,delExpense,editExpense,addProxy,delProxy,toggleProxy,markPaid,convertToExp,upd,restoreAll};
  ...
  if(recs.length){lines.push(`\n💸 代墊分攤`);if(mode==="detail")recs.forEach(r=>lines.push(`  ${r.note}　NT$${fmt(r.amount)}`));else lines.push(`  共${recs.length}筆　NT$${fmt(rTotal)}`);}
  if(prox.length){lines.push(`\n🛍️ 代購清單`);if(mode==="detail")prox.forEach(p=>{const sym=CURRS.find(c=>c.code===p.currency)?.sym??"";lines.push(`  ${p.item}${p.currency!=="TWD"?` (${sym}${fmt(p.price)}×${p.qty})`:`×${p.qty}`}　NT$${fmt(toTWD(p.price*p.qty,p.currency)+(p.feeTotal??0))}`);});else lines.push(`  共${prox.length}件　NT$${fmt(pTotal)}`);}
  lines.push(`\n━━━━━━━━━━━━━━━━━━━━`,`💰 合計應付　NT$${fmt(grand)}`);
...
function SettingsSheet({banks,setBanks,cards,setCards,friends,setFriends,trips,onRestore,onClose}){
  ...
  const handleBackup=()=>{
    const data={version:1,exportedAt:new Date().toISOString(),trips,banks,cards,friends};
    const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url; a.download=`旅行帳本備份_${new Date().toLocaleDateString("zh-TW").replace(/\//g,"-")}.json`;
  ...
  const handleRestore=(e)=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        if(!data.trips||!Array.isArray(data.trips)) throw new Error("格式錯誤");
        if(window.confirm(`確定還原備份？\n備份日期：${data.exportedAt?.slice(0,10)??""}\n共 ${data.trips.length} 趟旅行\n\n⚠️ 現有資料將被覆蓋`)){
          onRestore(data); setRestoreMsg("✅ 還原成功！"); setTimeout(()=>setRestoreMsg(null),3000);
        }
      }catch{setRestoreMsg("❌ 檔案格式不符");setTimeout(()=>setRestoreMsg(null),4000);}
    };
  ...
  return(
    <Sheet onClose={onClose}>
      ...
      {tab==="backup"&&<>
        <div style={{background:"#FFF8E8",border:"1px solid #F0D88A",borderRadius:14,padding:"12px 14px",marginBottom:20,fontSize:12,color:"#8A7040",lineHeight:1.7}}>⚠️ 清除瀏覽器資料會刪除帳本！建議每趟旅行結束後備份一次。</div>
        <div style={{background:C.card,borderRadius:18,border:`1px solid ${C.border}`,padding:"18px",marginBottom:14}}>
          <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:6}}>📤 匯出備份</div>
          <div style={{fontSize:12,color:C.textFaint,marginBottom:14,lineHeight:1.6}}>將所有旅行帳本打包下載成 .json 檔</div>
          <div style={{fontSize:12,color:C.textLight,marginBottom:14}}>目前資料：{trips.length} 趟旅行・{banks.length} 個帳號・{cards.length} 張信用卡</div>
          <Btn onClick={handleBackup} color={C.sage} style={{width:"100%"}}>💾 下載備份檔案</Btn>
        </div>
        <div style={{background:C.card,borderRadius:18,border:`1px solid ${C.border}`,padding:"18px",marginBottom:14}}>
          <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:6}}>📥 還原備份</div>
          <div style={{fontSize:12,color:C.textFaint,marginBottom:14,lineHeight:1.6}}>選擇備份的 .json 檔還原資料<br/><span style={{color:C.red,fontWeight:600}}>注意：還原後現有資料會被覆蓋</span></div>
          <label style={{display:"block",width:"100%",padding:"13px",borderRadius:16,border:`1.5px dashed ${C.rose}`,background:C.roseLight,color:C.rose,fontSize:14,fontWeight:600,cursor:"pointer",textAlign:"center",fontFamily:SANS}}>
            📂 選擇備份檔案<input type="file" accept=".json" onChange={handleRestore} style={{display:"none"}}/>
          </label>
        </div>
...
function EditExpenseSheet({expense,trip,friends,cards,onSave,onClose}){
  ...
  const save=()=>{
    if(!f.amount||isNaN(f.amount)||+f.amount<=0) return;
    const patch={...f,amount:+f.amount,myShare:f.iParticipate?myShare:0,friendShares,feeTotal};
    const recs=f.participants.map(fr=>({id:uid(),friend:fr,date:f.date,note:f.note||(CATS.find(c=>c.id===f.category)?.l??""),amount:friendShares[fr],paid:false}));
    onSave(patch,recs); onClose();
  };
  return(
    <Sheet onClose={onClose}>
      <SHead title="✏️ 編輯支出" onClose={onClose}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <div><Lbl ch="日期"/><Inp type="date" value={f.date} onChange={e=>set("date",e.target.value)}/></div>
        <div><Lbl ch="幣別"/><Sel value={f.currency} onChange={e=>set("currency",e.target.value)} style={{width:"100%"}}>{CURRS.map(c=><option key={c.code} value={c.code}>{c.code} {c.l}</option>)}</Sel></div>
      </div>
...
function TripDetail({trip,store,banks,cards,friends,onBack}){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg}}>
      ...
      {tab==="stats"&&<StatsTab trip={trip} onOpenPrint={m=>setPrintMode(m)} onExcel={()=>showToast({icon:"📊",title:`${trip.name}_結算表.xlsx`,lines:[`✦ 我的支出明細　${trip.expenses.length}筆`,`✦ 待收款清單　${(trip.receivables??[]).length}筆`,`✦ 代購清單　${(trip.proxies??[]).length}件`,`─────────────────`,`正式版整合 SheetJS 即可真實下載`]})}/>}
        ...
        return(<div key={date} style={{marginBottom:20}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {items.map(e=>{
              ...
              return(<Card key={e.id} s={{padding:"13px 15px",cursor:trip.archived?"default":"pointer"}} onClick={()=>!trip.archived&&onEdit&&onEdit(e)}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div style={{textAlign:"right",flexShrink:0}}>
                    ...
                    {!trip.archived&&<div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:3}}>
                      <span style={{fontSize:10,color:C.sage}}>✏️ 點卡片編輯</span>
                      <button onClick={ev=>{ev.stopPropagation();onDel(e.id);}} style={{background:"none",border:"none",fontSize:10,color:C.textFaint,cursor:"pointer",padding:0}}>刪除</button>
                    </div>}
                  </div>
                </div>
              </Card>);
        ...
        {editMembers&&(
          <div style={{padding:"0 16px 16px",borderTop:`1px solid ${C.border}`}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <input placeholder="新增旅伴姓名" value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&newName.trim()&&!friends.includes(newName.trim())){onFriendsChange([...friends,newName.trim()]);setNewName("");}}} style={{flex:1,padding:"10px 14px",borderRadius:12,border:`1.5px solid ${C.border}`,background:"#FDFCFB",fontSize:13,color:C.text,outline:"none",fontFamily:SANS}}/>
              <button onClick={()=>{if(newName.trim()&&!friends.includes(newName.trim())){onFriendsChange([...friends,newName.trim()]);setNewName("");}}} style={{padding:"10px 16px",borderRadius:12,border:"none",background:C.sage,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SANS,flexShrink:0}}>+ 新增</button>
            </div>
          </div>
        )}
      </div>
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(60,50,40,0.5)",backdropFilter:"blur(10px)",zIndex:500,overflowY:"auto"}}>
      <div style={{position:"sticky",top:0,zIndex:10,background:"rgba(249,248,246,0.96)",backdropFilter:"blur(12px)",borderBottom:`1px solid ${C.border}`,padding:"12px 16px",display:"flex",gap:10,alignItems:"center",maxWidth:680,margin:"0 auto"}}>
        ...
        <button onClick={doPrint} style={{background:C.rose,border:"none",borderRadius:12,padding:"10px 20px",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:SANS}}>🖨️ 列印 / PDF</button>
      </div>
      <div style={{maxWidth:680,margin:"0 auto",padding:"0 0 60px",background:"#fff"}}>
        <div style={{background:coverGrad,padding:"48px 36px 40px",color:"#fff",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{fontSize:12,opacity:0.7,marginBottom:24}}>{trip.startDate}{trip.endDate?` → ${trip.endDate}`:""} ・ {days} 天 ・ {trip.expenses.length} 筆</div>
        ...
        <div style={{padding:"28px 36px 0"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:32}}>
            {[{l:"旅行天數",v:`${days} 天`,c:"#F0E6E1",tc:C.rose},{l:"總支出筆數",v:`${trip.expenses.length} 筆`,c:"#E4EBE2",tc:C.sage},{l:"日均花費",v:`NT$${fmt(days?myTotal/days:0)}`,c:"#EDE6D8",tc:C.sand}].map(s=>(
          ...
          {byCat.length>0&&<>
            ...
            <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
              <PieChart data={byCat} size={180}/>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:32}}>
              {byCat.map(c=>(
                <div key={c.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:c.color+"15",borderRadius:12,borderLeft:`4px solid ${c.color}`}}>
                  <span style={{fontSize:13,color:C.text}}>{c.emoji} {c.label}</span>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:15,fontWeight:800,color:C.text}}>NT${fmt(c.value)}</div>
          ...
          {mode==="detail"&&<>
            <div style={{fontSize:11,color:C.textFaint,letterSpacing:2,fontWeight:600,marginBottom:18}}>支出明細</div>
            {dates.map(date=>{
              const items=grouped[date]; const dayMine=items.reduce((s,e)=>s+(e.myShare??0),0);
              return(<div key={date} style={{marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",paddingBottom:8,borderBottom:`2px solid ${C.roseLight}`,marginBottom:10}}>
                ...
                {items.map(e=>{
                  const cat=CATS.find(c=>c.id===e.category)||CATS[6]; const pm=PAYS.find(m=>m.id===e.payment);
                  return(<div key={e.id} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 10px",marginBottom:4,background:C.bg,borderRadius:10}}>
          ...
          {allFriends.length>0&&<>
            <div style={{fontSize:11,color:C.textFaint,letterSpacing:2,fontWeight:600,marginBottom:14}}>朋友待收明細</div>
            {allFriends.map(f=>{
              const items=(trip.receivables??[]).filter(r=>r.friend===f&&!r.paid);
              if(!items.length) return null;
              const fTotal=items.reduce((s,r)=>s+r.amount,0);
              return(<div key={f} style={{marginBottom:16,background:C.greenLight,borderRadius:16,padding:"14px 16px"}}>
