...
function useStore(){
  ...
  const addExpense=(tid,e,recs=[])=>{setTrips(prev=>prev.map(x=>{
    ...
    let newWallets=[...(x.cashWallets??[])];
    if(e.payment==="cash"){const idx=newWallets.findIndex(w=>w.currency===e.currency);if(idx>=0)newWallets[idx]={...newWallets[idx],amount:Math.max(0,newWallets[idx].amount-e.amount)};}
    return{...x,expenses:[{...e,id:uid()},...(x.expenses??[])],receivables:[...recs,...(x.receivables??[])],cashWallets:newWallets};
  }));};
  ...
  const buySelfWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{
    setTrips(prev=>prev.map(t=>{
      ...
      let newWallets=[...(t.cashWallets??[])];
      if(payment==="cash"){const idx=newWallets.findIndex(w=>w.currency===currency);if(idx>=0)newWallets[idx]={...newWallets[idx],amount:Math.max(0,newWallets[idx].amount-price*qty)};}
      return{...t,selflist:(t.selflist??[]).map(x=>x.id===wid?{...x,done:true}:x),expenses:[ne,...(t.expenses??[])],cashWallets:newWallets};
    }));
  };
  ...
  const addExchange=(tid,ex)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;let newW=[...(t.cashWallets??[])];const fromIdx=newW.findIndex(w=>w.currency===ex.fromCurrency);if(fromIdx>=0)newW[fromIdx]={...newW[fromIdx],amount:Math.max(0,newW[fromIdx].amount-ex.fromAmount)};const toIdx=newW.findIndex(w=>w.currency===ex.toCurrency);if(toIdx>=0)newW[toIdx]={...newW[toIdx],amount:newW[toIdx].amount+ex.toAmount};else newW=[...newW,{id:uid(),currency:ex.toCurrency,amount:ex.toAmount}];return{...t,exchanges:[{...ex,id:uid()},...(t.exchanges??[])],cashWallets:newW};}));};
  const delExchange=(tid,eid)=>{setTrips(prev=>prev.map(t=>t.id===tid?{...t,exchanges:(t.exchanges??[]).filter(e=>e.id!==eid)}:t));};
...
function AddExpenseSheet({trip,friends,cards,allCurrs,onSave,onClose,prefill,apiKey}){
  const preferCurrs=trip.preferCurrs??["TWD"];
  ...
  const save=()=>{
    ...
    const exp={...f,amount:+f.amount,myShare:f.iParticipate?myShare:0,friendShares,feeTotal,platformLabel,cardName};
...
function EditExpenseSheet({expense,trip,friends,cards,allCurrs,onSave,onClose}){
  ...
  const save=()=>{
    ...
    const platformLabel=f.platform==="manual"?f.platformManual:(PLATFORMS.find(p=>p.id===f.platform)?.l??"");
    const cardName=selCard?.name??"";
    const patch={...f,amount:+f.amount,myShare:f.iParticipate?myShare:0,friendShares,feeTotal,platformLabel,cardName};
    const recs=f.participants.map(fr=>({id:uid(),friend:fr,date:f.date,note:f.note||(CATS.find(c=>c.id===f.category)?.l??""),amount:friendShares[fr],paid:false}));
  ...
  return(<div style={{padding:"0 16px 100px"}}>
    ...
    {active.length>0&&<><div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:12}}>進行中</div>
      {active.map(t=>{
        ...
        const walletStr=wallets.filter(w=>w.amount>0).map(w=>{const curr=[...BASE_CURRS,...(allCurrs.filter(c=>!BASE_CURRS.find(b=>b.code===c.code)))].find(c=>c.code===w.currency);return`${curr?.sym??w.currency}${Math.round(w.amount).toLocaleString()}`;}).join(" · ");
...
function TripDetail({trip,store,banks,cards,friends,allCurrs,onBack}){
  ...
  return(<div style={{minHeight:"100vh",background:"#F9F8F6"}}>
    ...
    <div style={{height:40}}/>
    {tab==="ledger"&&<LedgerTab trip={trip} allCurrs={allCurrs} onAdd={()=>openExp(null)} onAddWithScan={prefill=>openExp(prefill)} onDel={eid=>store.delExpense(trip.id,eid)} onEdit={e=>setEditingExp(e)} apiKey={store.apiKey} preferCurrs={trip.preferCurrs??["TWD"]} cards={cards}/>}
    ...
    {modal==="exp"&&<AddExpenseSheet trip={trip} friends={friends} cards={cards} allCurrs={allCurrs} onSave={(exp,recs)=>store.addExpense(trip.id,exp,recs)} onClose={()=>{setModal(null);setExpPrefill(null);}} prefill={expPrefill} apiKey={store.apiKey}/>}
...
function LedgerTab({trip,allCurrs,onAdd,onAddWithScan,onDel,onEdit,apiKey,preferCurrs,cards}){
    ...
    :dates.map(date=>{const items=grouped[date];const dayMine=items.reduce((s,e)=>s+(e.myShare??0),0);
      return(<div key={date} style={{marginBottom:20}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {items.map(e=>{const cat=CATS.find(c=>c.id===e.category)||CATS[CATS.length-1];const pm=PAYS.find(m=>m.id===e.payment);const currSym=allCurrs.find(c=>c.code===e.currency)?.sym??"";const platLabel=e.platformLabel||(e.platform&&e.platform!=="manual"?(PLATFORMS.find(p=>p.id===e.platform)?.l??""):"");const cardName=e.cardName||(e.cardId?(cards.find(c=>c.id===e.cardId)?.name??""):"");
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",gap:5,marginTop:3,flexWrap:"wrap",alignItems:"center"}}>
                    ...
                    {cardName&&<span style={{fontSize:10,background:"#EDE6D8",color:"#C8B89A",padding:"1px 7px",borderRadius:99}}>💳 {cardName}</span>}
  ...
  return(<div style={{padding:"16px 16px 100px"}}>
    ...
    {subTab==="wallet"&&<>
      ...
      {showAddWallet&&<div style={{background:"#FFFFFF",borderRadius:18,border:"1px solid #EDEBE6",padding:"16px",marginBottom:14}}>
        ...
        <Lbl ch="金額"/><Inp type="number" placeholder="0" value={newW.amount} onChange={e=>setNewW(p=>({...p,amount:e.target.value}))} style={{marginBottom:12}}/>
      ...
      {wallets.map(w=>{const curr=allCurrs.find(c=>c.code===w.currency);const twdVal=w.amount*(curr?.r??1);
        return(<div key={w.id} style={{background:"#FFFFFF",borderRadius:20,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:13,color:"#8A8A7A"}}>{curr?.l??w.currency}</div>
              {editingWid===w.id
              ...
              <div style={{fontSize:12,color:"#BBBBA8"}}>≈ NT${fmt(twdVal)}</div>
            </div>
    ...
    {subTab==="exchange"&&<>
      ...
      {exchanges.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:"#BBBBA8"}}><div style={{fontSize:36,marginBottom:8}}>💱</div><div style={{fontFamily:SERIF,fontSize:14}}>還沒有換匯紀錄</div></div>}
      {exchanges.map(ex=>{const rate=(+ex.toAmount/+ex.fromAmount).toFixed(4);
        return(<div key={ex.id} style={{background:"#FFFFFF",borderRadius:20,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            {!trip.archived&&<button onClick={()=>{if(window.confirm("確定刪除此換匯紀錄？"))onDelExchange(ex.id);}} style={{padding:"7px 10px",borderRadius:9,border:"1px solid #FAE8E3",background:"#FAE8E3",color:"#D4806A",fontSize:11,cursor:"pointer"}}>🗑</button>}
          </div>
        </div>);
      })}
    </>}
...
function StatsTab({trip,allCurrs,cards,onExcel,onOpenPrint}){
  ...
  return(<div style={{padding:"16px 16px 100px"}}>
    ...
    {wallets.length>0&&<div style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:12}}>
      ...
      {wallets.map(w=>{const curr=allCurrs.find(c=>c.code===w.currency);const twdVal=w.amount*(curr?.r??1);return(<div key={w.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,color:"#5A5A5A"}}>{curr?.sym} {fmtD(w.amount,0)} {w.currency}</span><span style={{fontSize:12,color:"#8A8A7A"}}>≈ NT${fmt(twdVal)}</span></div>);})}
