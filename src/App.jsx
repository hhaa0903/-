...
function useStore(){
  ...
  const addExpense=(tid,e,recs=[])=>{setTrips(prev=>prev.map(x=>{
    if(x.id!==tid)return x;
    const newId=uid();
    let w=[...(x.cashWallets??[])];
    if(e.payment==="cash")w=_adj(w,e.currency,-e.amount);
    const recsWithId=recs.map(r=>({...r,expenseId:newId}));
    return{...x,expenses:[{...e,id:newId},...(x.expenses??[])],receivables:[...recsWithId,...(x.receivables??[])],cashWallets:w};
  }));};

  const delExpense=(tid,eid)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const exp=(t.expenses??[]).find(e=>e.id===eid);
    let w=[...(t.cashWallets??[])];
    if(exp?.payment==="cash")w=_adj(w,exp.currency,+exp.amount);
    const recsToDelete=(t.receivables??[]).filter(r=>r.expenseId===eid);
    recsToDelete.forEach(r=>{
      if(r.paid&&r.repayType==="cash"&&r.repaymentCurrency&&r.repaymentAmount>0)
        w=_adj(w,r.repaymentCurrency,-r.repaymentAmount);
    });
    return{...t,expenses:(t.expenses??[]).filter(e=>e.id!==eid),receivables:(t.receivables??[]).filter(r=>r.expenseId!==eid),cashWallets:w};
  }));};

  const editExpense=(tid,eid,patch,newRecs=[])=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const old=(t.expenses??[]).find(e=>e.id===eid);
    let w=[...(t.cashWallets??[])];
    if(old?.payment==="cash")w=_adj(w,old.currency,+old.amount);
    if(patch.payment==="cash")w=_adj(w,patch.currency,-patch.amount);
    // 保留已還款紀錄，已還款的朋友不重複產生新紀錄
    const paidRecs=(t.receivables??[]).filter(r=>r.expenseId===eid&&r.paid);
    const otherRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);
    const paidFriends=new Set(paidRecs.map(r=>r.friend));
    const recsWithId=newRecs.filter(r=>!paidFriends.has(r.friend)).map(r=>({...r,expenseId:eid}));
    return{...t,
      expenses:(t.expenses??[]).map(e=>e.id===eid?{...e,...patch,id:eid}:e),
      receivables:[...recsWithId,...paidRecs,...otherRecs],
      cashWallets:w
    };
  }));};
  ...
  const buyWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    ...
    let wallets=[...(t.cashWallets??[])];
    if(payment==="cash")wallets=_adj(wallets,currency,-price*qty);
    return{...t,wishlist:(t.wishlist??[]).map(x=>x.id===wid?{...x,done:true}:x),proxies:[proxy,...(t.proxies??[])],cashWallets:wallets};
  }));};
  ...
  const buySelfWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    ...
    let wallets=[...(t.cashWallets??[])];
    if(payment==="cash")wallets=_adj(wallets,currency,-price*qty);
    return{...t,selflist:(t.selflist??[]).map(x=>x.id===wid?{...x,done:true}:x),expenses:[ne,...(t.expenses??[])],cashWallets:wallets};
  }));};

  const addProxy=(tid,p)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    let wallets=[...(t.cashWallets??[])];
    if(p.payment==="cash")wallets=_adj(wallets,p.currency,-p.price*p.qty);
    return{...t,proxies:[{...p,id:uid(),paid:false},...(t.proxies??[])],cashWallets:wallets};
  }));};
  const delProxy=(tid,pid)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const proxy=(t.proxies??[]).find(p=>p.id===pid);
    let wallets=[...(t.cashWallets??[])];
    if(proxy?.payment==="cash")wallets=_adj(wallets,proxy.currency,proxy.price*proxy.qty);
    return{...t,proxies:(t.proxies??[]).filter(p=>p.id!==pid),cashWallets:wallets};
  }));};
  const toggleProxy=(tid,pid)=>{setTrips(prev=>prev.map(t=>t.id===tid?{...t,proxies:(t.proxies??[]).map(p=>p.id===pid?{...p,paid:!p.paid}:p)}:t));};

  const markManyPaid=(tid,rids,repayType,repaymentCurrency,repaymentAmount)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    let wallets=[...(t.cashWallets??[])];
    if(repayType==="cash"&&repaymentCurrency&&repaymentAmount>0){
      const toIdx=wallets.findIndex(w=>w.currency===repaymentCurrency);
      if(toIdx>=0)wallets=_adj(wallets,repaymentCurrency,repaymentAmount);
      else wallets=[...wallets,{id:uid(),currency:repaymentCurrency,amount:repaymentAmount,initialAmount:0}];
    }
    const newRecs=(t.receivables??[]).map(r=>rids.includes(r.id)?{...r,paid:true,repayType:repayType??"transfer",repaymentCurrency:repaymentCurrency??"",repaymentAmount:repaymentAmount??0}:r);
    return{...t,receivables:newRecs,cashWallets:wallets};
  }));};

  const unmarkPaid=(tid,rid)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const rec=(t.receivables??[]).find(r=>r.id===rid);
    let wallets=[...(t.cashWallets??[])];
    if(rec?.paid&&rec?.repayType==="cash"&&rec?.repaymentCurrency&&rec?.repaymentAmount>0)
      wallets=_adj(wallets,rec.repaymentCurrency,-rec.repaymentAmount);
    return{...t,receivables:(t.receivables??[]).map(r=>r.id===rid?{...r,paid:false,repayType:null,repaymentCurrency:null,repaymentAmount:0}:r),cashWallets:wallets};
  }));};

  const delRec=(tid,rid)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const rec=(t.receivables??[]).find(r=>r.id===rid);
    let wallets=[...(t.cashWallets??[])];
    if(rec?.paid&&rec?.repayType==="cash"&&rec?.repaymentCurrency&&rec?.repaymentAmount>0)
      wallets=_adj(wallets,rec.repaymentCurrency,-rec.repaymentAmount);
    return{...t,receivables:(t.receivables??[]).filter(r=>r.id!==rid),cashWallets:wallets};
  }));};
  ...
  const convertToExp=(tid,rid)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const r=(t.receivables??[]).find(x=>x.id===rid);if(!r)return t;
    const ne={id:uid(),date:todayStr(),category:"other",note:`${r.friend} 未還款`,amount:r.amount,currency:"TWD",payment:"cash",iParticipate:true,participants:[],myShare:r.amount,friendShares:{},feeTotal:0,isOverseas:false};
    return{...t,expenses:[ne,...t.expenses],receivables:(t.receivables??[]).map(x=>x.id===rid?{...x,paid:true,converted:true}:x)};
  }));};

  const addCashWallet=(tid,wallet)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const existing=(t.cashWallets??[]).findIndex(w=>w.currency===wallet.currency);
    if(existing>=0){const nw=[...(t.cashWallets??[])];nw[existing]={...nw[existing],amount:nw[existing].amount+wallet.amount,initialAmount:(nw[existing].initialAmount??nw[existing].amount)+wallet.amount};return{...t,cashWallets:nw};}
    return{...t,cashWallets:[...(t.cashWallets??[]),{...wallet,id:uid(),initialAmount:wallet.amount}]};
  }));};
  ...
  const addExchange=(tid,ex)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    let w=[...(t.cashWallets??[])];
    w=_adj(w,ex.fromCurrency,-ex.fromAmount);
    const toIdx=w.findIndex(x=>x.currency===ex.toCurrency);
    if(toIdx>=0)w[toIdx]={...w[toIdx],amount:w[toIdx].amount+ex.toAmount};
    else w=[...w,{id:uid(),currency:ex.toCurrency,amount:ex.toAmount,initialAmount:ex.toAmount}];
    return{...t,exchanges:[{...ex,id:uid()},...(t.exchanges??[])],cashWallets:w};
  }));};
  const delExchange=(tid,eid)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    const ex=(t.exchanges??[]).find(e=>e.id===eid);if(!ex)return t;
    let w=[...(t.cashWallets??[])];
    w=_adj(w,ex.fromCurrency,+ex.fromAmount);
    w=_adj(w,ex.toCurrency,-ex.toAmount);
    return{...t,exchanges:(t.exchanges??[]).filter(e=>e.id!==eid),cashWallets:w};
  }));};
...
function PrintReport({trip,mode,allCurrs,cards,onClose}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(60,50,40,0.5)",backdropFilter:"blur(10px)",zIndex:500,overflowY:"auto"}}>
    <style>{`@media print{body *{visibility:hidden}#tf-print-area,#tf-print-area *{visibility:visible}#tf-print-area{position:absolute;left:0;top:0;width:100%;overflow:visible!important}}`}</style>
