...
function useStore(){
  ...
  if(recs.length){
    lines.push("\n💸 代墊分攤");
    if(mode==="detail")recs.forEach(r=>{
      const curr=allCurrs.find(c=>c.code===r.currency);
      const origStr=(r.currency&&r.currency!=="TWD"&&r.origAmount)?` (${curr?.sym??r.currency}${fmtD(r.origAmount,0)})`:"";
      lines.push(`  ${r.note}${origStr}　NT$${fmt(r.amount)}`);
    });
  ...
  if(prox.length){
    ...
    if(mode==="detail")prox.forEach(p=>{
      const curr=allCurrs.find(c=>c.code===p.currency);
      const sym=curr?.sym??p.currency;
      lines.push(`  ${p.item} ×${p.qty}　${sym}${fmtD(p.price,0)} → NT$${fmt(p.totalTWD??0)}`);
    });
...
function SettingsSheet({banks,setBanks,cards,setCards,friends,setFriends,trips,customCurrs,setCustomCurrs,customPays,setCustomPays,apiKey,setApiKey,onRestore,onClose}){
  ...
  const [np,setNp]=useState({name:"",e:"💳",feeRateOverseas:"",feeRateDomestic:"0"});
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(60,50,40,0.5)",backdropFilter:"blur(10px)",zIndex:500,overflowY:"auto"}}>
    <style>{`
      @media print {
        ...
        #tf-print-area { display: block !important; position: static !important; overflow: visible !important; width: 100% !important; }
        #tf-print-area * { visibility: visible !important; }
        .no-print { display: none !important; }
      }
    `}</style>
    <div className="no-print" style={{position:"sticky",top:0,zIndex:10,background:"rgba(249,248,246,0.96)",backdropFilter:"blur(12px)",borderBottom:"1px solid #EDEBE6",padding:"12px 16px",display:"flex",gap:10,alignItems:"center",maxWidth:680,margin:"0 auto"}}>
      ...
      <div style={{flex:1,fontSize:13,color:"#BBBBA8"}}>預覽・{mode==="detail"?"詳細版（全部明細）":"簡式版"}</div>
