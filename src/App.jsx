...
function useStore(){
  ...
  const addExpense=(tid,e,recs=[])=>{setTrips(prev=>prev.map(x=>{
    ...
    return{...x,expenses:[{...e,id:uid()},...(x.expenses??[])],receivables:[...recs,...(x.receivables??[])],cashWallets:w};
  }));};
  ...
  const markManyPaid=(tid,rids,repayType,repaymentCurrency,repaymentAmount)=>{setTrips(prev=>prev.map(t=>{
    ...
    if(repayType==="cash"&&repaymentCurrency&&repaymentAmount>0){
      wallets=_adj(wallets,repaymentCurrency,repaymentAmount);
      // 若該幣別錢包不存在，新增
      if(!wallets.find(w=>w.currency===repaymentCurrency)){
        wallets=[...wallets,{id:uid(),currency:repaymentCurrency,amount:repaymentAmount,initialAmount:0}];
      }
    }
    const newRecs=(t.receivables??[]).map(r=>rids.includes(r.id)?{...r,paid:true,repayType:repayType??"transfer",repaymentCurrency:repaymentCurrency??"TWD",repaymentAmount:repaymentAmount??0}:r);
...
function LedgerTab({trip,allCurrs,cards,customPays,onAdd,onAddWithScan,onDel,onEdit,apiKey,preferCurrs}){
  ...
  const handleRepayConfirm=()=>{
    ...
    setChecked({});setShowRepaySheet(false);setRepayAmtInput("");
  };
  return(<div style={{padding:"16px 16px 100px"}}>
    ...
    {checkedIds.length>0&&<div style={{background:"#FFFFFF",borderRadius:16,border:"1.5px solid #88B098",padding:"12px 16px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <Btn sm color="#88B098" onClick={()=>setShowRepaySheet(true)}>✓ 還款設定</Btn>
      </div>
      {showRepaySheet&&<div style={{borderTop:"1px solid #EDEBE6",paddingTop:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <Btn outline color="#BBBBA8" onClick={()=>setShowRepaySheet(false)} style={{flex:1}}>取消</Btn>
        </div>
      </div>}
    </div>}
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(60,50,40,0.5)",backdropFilter:"blur(10px)",zIndex:500,overflowY:"auto"}}>
    ...
    <div id="tf-print-area" style={{maxWidth:680,margin:"0 auto",padding:"0 0 60px",background:"#fff"}}>
      ...
      <div style={{padding:"28px 36px 0"}}>
        ...
        {mode==="detail"&&<>
          <div style={{fontSize:12,fontWeight:700,color:"#C9A89A",marginBottom:16,paddingBottom:6,borderBottom:"2px solid #F0E6E1"}}>📖 完整支出明細</div>
