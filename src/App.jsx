...
async function scanReceipt(base64Img,apiKey,preferCurrs,allCurrs,customCurrs){
...
請回傳以下JSON格式（只回傳JSON，不要其他文字）：
{
  ...
  "note": "品項或店名（用中文或原文）",
  "category": "food或transport或hotel或shopping或activity或beauty或other",
  "confidence": "high或medium或low"
}

辨識規則：
- 金額取「合計」「総計」「합계」「total」「รวม」等最終金額
- 幣別從收據語言、符號、金額判斷，優先從常用幣別中選
- 看到円/¥/税込 → JPY；₩/원 → KRW；฿/บาท → THB；等
- category判斷：餐廳/食物→food；計程車/捷運→transport；飯店→hotel；百貨/藥妝→shopping；門票/體驗→activity；美妝/保養→beauty
- 若辨識不確定請設confidence為low`;
  const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-opus-4-6",max_tokens:500,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:base64Img.split(",")[1]}},{type:"text",text:prompt}]}]})});
  const data=await res.json();
  const text=data.content?.[0]?.text??"{}";
  const clean=text.replace(/```json|```/g,"").trim();
  return JSON.parse(clean);
}
function ReceiptScanBtn({apiKey,preferCurrs,allCurrs,customCurrs,onResult}){
  ...
  const handleScan=async e=>{
    ...
    try{
      ...
      const result=await scanReceipt(b64,apiKey,preferCurrs,allCurrs,customCurrs);
      onResult(result);
    }catch(ex){setErr("辨識失敗，請重試或手動記帳");}
...
function useStore(){
  ...
  const buySelfWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{
    setTrips(prev=>prev.map(t=>{
      ...
      const ne={id:uid(),date:todayStr(),category:"shopping",note:w.item,amount:price*qty,currency,payment,cardId,iParticipate:true,participants:[],myShare,friendShares:{},feeTotal};
...
function SettingsSheet({banks,setBanks,cards,setCards,friends,setFriends,trips,customCurrs,setCustomCurrs,apiKey,setApiKey,onRestore,onClose}){
  ...
  const [keyInput,setKeyInput]=useState(apiKey);const [keyVisible,setKeyVisible]=useState(false);
  ...
  return(<Sheet onClose={onClose}><SHead title="⚙️ 設定" onClose={onClose}/>
    ...
    {tab==="ai"&&<>
      <div style={{background:"#EAE6F2",borderRadius:14,padding:"12px 14px",marginBottom:16,fontSize:12,color:"#8A78A8",lineHeight:1.8}}>
        🤖 使用 Anthropic Claude AI 辨識收據<br/>
        支援日文、韓文、泰文、英文等多國語言<br/>
        幣別辨識會優先匹配旅行的常用幣別設定
      </div>
      ...
      <div style={{background:"#FFF8E8",border:"1px solid #F0D88A",borderRadius:14,padding:"12px 14px",marginTop:12,fontSize:11,color:"#8A7040",lineHeight:1.7}}>
        ⚠️ API Key 只存在你的手機，不會上傳任何伺服器<br/>
        取得方式：前往 console.anthropic.com 申請
      </div>
    </>}
...
function AddExpenseSheet({trip,friends,cards,allCurrs,onSave,onClose,prefill,apiKey,customCurrs}){
  const preferCurrs=trip.preferCurrs??["TWD"];
  const [f,setF]=useState({date:todayStr(),category:(prefill?.category??"food"),note:(prefill?.note??""),amount:(prefill?.amount??""),currency:(prefill?.currency??(preferCurrs[0]??"TWD")),payment:"cash",cardId:"",iParticipate:true,participants:[]});
  ...
  return(<Sheet onClose={onClose}><SHead title="新增支出" onClose={onClose}/>
    {!scanDone&&<ReceiptScanBtn apiKey={apiKey} preferCurrs={preferCurrs} allCurrs={allCurrs} customCurrs={customCurrs} onResult={handleScanResult}/>}
