...
function useStore(){
  ...
  const createTrip=d=>{const t={id:uid(),createdAt:todayStr(),archived:false,expenses:[],proxies:[],wishlist:[],selflist:[],receivables:[],exchanges:[],cashWallets:[],...d};setTrips(p=>[t,...p]);setActiveId(t.id);return t.id;};
  ...
  const _adjustWallet=(wallets,currency,delta)=>{
    const idx=wallets.findIndex(w=>w.currency===currency);
    if(idx<0)return wallets;
    const newW=[...wallets];
    newW[idx]={...newW[idx],amount:Math.max(0,newW[idx].amount+delta)};
    return newW;
  };

  const addExpense=(tid,e,recs=[])=>{setTrips(prev=>prev.map(x=>{
    if(x.id!==tid)return x;
    let wallets=[...(x.cashWallets??[])];
    if(e.payment==="cash")wallets=_adjustWallet(wallets,e.currency,-e.amount);
    return{...x,expenses:[{...e,id:uid()},...(x.expenses??[])],receivables:[...recs,...(x.receivables??[])],cashWallets:wallets};
  }));};
  ...
  const delExpense=(tid,eid)=>{setTrips(prev=>prev.map(t=>{
    ...
    if(exp&&exp.payment==="cash")wallets=_adjustWallet(wallets,exp.currency,+exp.amount);
    return{...t,expenses:(t.expenses??[]).filter(e=>e.id!==eid),cashWallets:wallets};
  }));};
  ...
  const editExpense=(tid,eid,patch,newRecs=[])=>{setTrips(prev=>prev.map(t=>{
    ...
    let wallets=[...(t.cashWallets??[])];
    if(old&&old.payment==="cash")wallets=_adjustWallet(wallets,old.currency,+old.amount);
    if(patch.payment==="cash")wallets=_adjustWallet(wallets,patch.currency,-patch.amount);
    const filteredRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);
    const recsWithId=newRecs.map(r=>({...r,expenseId:eid}));
    return{...t,expenses:(t.expenses??[]).map(e=>e.id===eid?{...e,...patch,id:eid}:e),receivables:[...recsWithId,...filteredRecs],cashWallets:wallets};
  }));};
  ...
  const buySelfWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{
    setTrips(prev=>prev.map(t=>{
      ...
      let wallets=[...(t.cashWallets??[])];
      if(payment==="cash")wallets=_adjustWallet(wallets,currency,-price*qty);
  ...
  const addCashWallet=(tid,wallet)=>{setTrips(prev=>prev.map(t=>{
    ...
    if(existing>=0){
      const newW=[...(t.cashWallets??[])];
      newW[existing]={...newW[existing],amount:newW[existing].amount+wallet.amount,initialAmount:(newW[existing].initialAmount??newW[existing].amount)+wallet.amount};
      return{...t,cashWallets:newW};
    }
  ...
  const addExchange=(tid,ex)=>{setTrips(prev=>prev.map(t=>{
    if(t.id!==tid)return t;
    let wallets=[...(t.cashWallets??[])];
    wallets=_adjustWallet(wallets,ex.fromCurrency,-ex.fromAmount);
    const toIdx=wallets.findIndex(w=>w.currency===ex.toCurrency);
    if(toIdx>=0)wallets[toIdx]={...wallets[toIdx],amount:wallets[toIdx].amount+ex.toAmount};
    else wallets=[...wallets,{id:uid(),currency:ex.toCurrency,amount:ex.toAmount,initialAmount:ex.toAmount}];
    return{...t,exchanges:[{...ex,id:uid()},...(t.exchanges??[])],cashWallets:wallets};
  }));};
  ...
  const delExchange=(tid,eid)=>{setTrips(prev=>prev.map(t=>{
    ...
    let wallets=[...(t.cashWallets??[])];
    wallets=_adjustWallet(wallets,ex.fromCurrency,+ex.fromAmount);
    wallets=_adjustWallet(wallets,ex.toCurrency,-ex.toAmount);
    return{...t,exchanges:(t.exchanges??[]).filter(e=>e.id!==eid),cashWallets:wallets};
  }));};
  ...
  return{trips,activeId,setActiveId,active,banks,setBanks,cards,setCards,friends,setFriends,customCurrs,setCustomCurrs,customPays,setCustomPays,allCurrs,allPays,apiKey,setApiKey,createTrip,archiveTrip,reopenTrip,deleteTrip,addExpense,delExpense,editExpense,addWish,delWish,buyWish,addSelfWish,delSelfWish,buySelfWish,addProxy,delProxy,toggleProxy,markManyPaid,unmarkPaid,delRec,markManyProxyPaid,convertToExp,addCashWallet,updateCashWallet,delCashWallet,addExchange,delExchange,upd,restoreAll};
  ...
  return(<Sheet onClose={onClose}><SHead title="⚙️ 設定" onClose={onClose}/>
    ...
    {tab==="cards"&&<>
      ...
      {cards.map(c=><div key={c.id} style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:14,fontWeight:600,color:"#5A5A5A"}}>💳 {c.name}</div><div style={{fontSize:12,color:"#8A8A7A"}}>海外 {c.feeRateOverseas??c.feeRate??0}%・國內 {c.feeRateDomestic??0}%</div></div>
    ...
    {tab==="pays"&&<>
      <Lbl ch="自訂支付方式"/>
      <div style={{background:"#E4EBE2",borderRadius:14,padding:"10px 14px",marginBottom:12,fontSize:12,color:"#6A8A7A",lineHeight:1.7}}>💡 新增 Apple Pay、全支付等行動支付，可設定海外與國內手續費</div>
      <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
        {EMOJI_OPTS.map(e=><button key={e} onClick={()=>setNp(p=>({...p,e}))} style={{width:36,height:36,borderRadius:10,border:`1.5px solid ${np.e===e?"#C9A89A":"#EDEBE6"}`,background:np.e===e?"#F0E6E1":"transparent",fontSize:18,cursor:"pointer"}}>{e}</button>)}
      </div>
      <Inp placeholder="支付方式名稱（如 Apple Pay）" value={np.name} onChange={e=>setNp(p=>({...p,name:e.target.value}))} style={{marginBottom:8}}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
        <div><Lbl ch="海外手續費 %"/><Inp placeholder="1.5" type="number" value={np.feeRateOverseas} onChange={e=>setNp(p=>({...p,feeRateOverseas:e.target.value}))}/></div>
        <div><Lbl ch="國內手續費 %"/><Inp placeholder="0" type="number" value={np.feeRateDomestic} onChange={e=>setNp(p=>({...p,feeRateDomestic:e.target.value}))}/></div>
      </div>
      <Btn onClick={addPay} style={{width:"100%",marginBottom:16}}>+ 新增支付方式</Btn>
      {customPays.map(p=><div key={p.id} style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:14,fontWeight:600,color:"#5A5A5A"}}>{p.e} {p.name}</div><div style={{fontSize:12,color:"#8A8A7A"}}>海外 {p.feeRateOverseas}%・國內 {p.feeRateDomestic}%</div></div>
        <button onClick={()=>setCustomPays(prev=>prev.filter(x=>x.id!==p.id))} style={{background:"none",border:"none",color:"#BBBBA8",cursor:"pointer",fontSize:18}}>🗑</button>
      </div>)}
      {customPays.length===0&&<div style={{textAlign:"center",padding:"20px 0",color:"#BBBBA8",fontSize:13}}>還沒有自訂支付方式</div>}
    </>}
...
function PaymentSel({value,cardId,onChange,onCardChange,cards,customPays,isOverseas,amtTWD}){
  ...
  return(<div>
    ...
    {value==="card"&&<div style={{marginBottom:10}}>
      {cards.length===0?<div style={{fontSize:12,color:"#BBBBA8",padding:"8px 0"}}>請先在設定新增信用卡</div>
      :<div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:6}}>{cards.map(c=><Pill key={c.id} active={cardId===c.id} color="#B5A8C8" onClick={()=>onCardChange(c.id)}>💳 {c.name}</Pill>)}</div>}
      {selCard&&amtTWD>0&&<div style={{background:"#EDE6D8",borderRadius:12,padding:"8px 14px",fontSize:12,color:"#C8B89A",fontWeight:600}}>{isOverseas?"海外":"國內"}手續費 {feeRate}%：NT${fmt(feeTotal)}</div>}
    </div>}
    {customPays.length>0&&<div style={{marginBottom:8}}>
      <button onClick={()=>setShowOther(p=>!p)} style={{background:"none",border:"none",fontSize:12,color:"#A8B5A2",cursor:"pointer",fontFamily:SANS,padding:"4px 0"}}>{showOther?"▲ 收起其他支付":"▼ 其他支付方式"}</button>
      {showOther&&<div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:8}}>
        {customPays.map(p=><Pill key={p.id} active={value===p.id} color="#A8B5A2" onClick={()=>onChange(p.id)}>{p.e} {p.name}</Pill>)}
      </div>}
      {selCustomPay&&amtTWD>0&&customFeeRate>0&&<div style={{background:"#E4EBE2",borderRadius:12,padding:"8px 14px",fontSize:12,color:"#A8B5A2",fontWeight:600,marginTop:6}}>{isOverseas?"海外":"國內"}手續費 {customFeeRate}%：NT${fmt(customFeeTotal)}</div>}
    </div>}
  </div>);
}
...
function AddExpenseSheet({trip,friends,cards,allCurrs,customPays,onSave,onClose,prefill,apiKey}){
  ...
  const save=()=>{
    ...
    const cardName=selCard?.name??"";
    const payName=selCustomPay?.name??"";
...
function TripList({trips,activeId,onSelect,onCreate,onArchive,onReopen,onDelete,onSettings,allCurrs}){
  ...
  return(<div style={{padding:"0 16px 100px"}}>
    ...
    {active.length>0&&<><div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:12}}>進行中</div>
      {active.map(t=>{
        const cover=cv(t);
        const myT=t.expenses.reduce((s,e)=>s+(e.myShare??0),0);
        const pend=(t.receivables??[]).filter(r=>!r.paid).reduce((s,r)=>s+r.amount,0)+((t.proxies??[]).filter(p=>!p.paid).reduce((s,p)=>s+(p.totalTWD??0),0));
        const wishPending=(t.wishlist??[]).filter(w=>!w.done).length;
        const selfPending=(t.selflist??[]).filter(w=>!w.done).length;
...
function TripDetail({trip,store,banks,cards,friends,allCurrs,onBack}){
  ...
  return(<div style={{minHeight:"100vh",background:"#F9F8F6"}}>
    ...
    {tab==="stats"&&<StatsTab trip={trip} allCurrs={allCurrs} cards={cards} onOpenPrint={m=>setPrintMode(m)} onExcel={()=>showToast({icon:"📊",title:`${trip.name}_結算表`,lines:[`✦ 支出 ${trip.expenses.length}筆`,`✦ 代墊 ${(trip.receivables??[]).length}筆`,`✦ 代購 ${(trip.proxies??[]).length}件`,"─────────────────","整合SheetJS即可真實下載"]})}/>}
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              ...
              {editingWid===w.id
                ...
                :<div>
                  <div style={{fontSize:22,fontWeight:800,color:"#5A5A5A"}}>{fmtD(w.amount,0)} <span style={{fontSize:13,color:"#8A8A7A"}}>{w.currency}</span></div>
                  {w.initialAmount!=null&&w.initialAmount!==w.amount&&<div style={{fontSize:11,color:"#BBBBA8",marginTop:2}}>原始攜帶：{fmtD(w.initialAmount,0)} {w.currency}</div>}
                </div>}
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                <span style={{fontSize:14,fontWeight:700,color:"#D4806A"}}>{fmtD(ex.fromAmount,0)} {ex.fromCurrency}</span>
                <span style={{color:"#A8B5A2",fontSize:14}}>→</span>
                <span style={{fontSize:14,fontWeight:700,color:"#88B098"}}>{fmtD(ex.toAmount,0)} {ex.toCurrency}</span>
              </div>
...
function StatsTab({trip,allCurrs,cards,onExcel,onOpenPrint}){
  const myTotal=trip.expenses.reduce((s,e)=>s+(e.myShare??0),0);
  ...
  const [customDays,setCustomDays]=useState(trip.customDays??null);
  const [editDays,setEditDays]=useState(false);
  const [daysInput,setDaysInput]=useState("");
  const autodays=[...new Set(trip.expenses.map(e=>e.date))].length;
  const days=customDays??autodays;
  return(<div style={{padding:"16px 16px 100px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4,flexWrap:"wrap"}}>
        {editDays
          ?<div style={{display:"flex",gap:6,alignItems:"center"}}>
            ...
            <button onClick={()=>{const d=+daysInput;if(d>0){setCustomDays(d);trip.customDays=d;}setEditDays(false);}} style={{padding:"4px 10px",borderRadius:8,border:"none",background:"#88B098",color:"#fff",fontSize:11,cursor:"pointer",fontFamily:SANS}}>✓</button>
          ...
          :<div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:12,color:"#BBBBA8"}}>{trip.expenses.length}筆・{days}天{customDays?"（自訂）":"（自動）"}</span>
