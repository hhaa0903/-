...
function useStore(){
  ...
  const delExpense=(tid,eid)=>{setTrips(prev=>prev.map(t=>{
    ...
    const newRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);
    return{...t,expenses:(t.expenses??[]).filter(e=>e.id!==eid),receivables:newRecs,cashWallets:w};
  }));};
  ...
  const editExpense=(tid,eid,patch,newRecs=[])=>{setTrips(prev=>prev.map(t=>{
    ...
    const filteredRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);
    const recsWithId=newRecs.map(r=>({...r,expenseId:eid}));
    return{...t,expenses:(t.expenses??[]).map(e=>e.id===eid?{...e,...patch,id:eid}:e),receivables:[...recsWithId,...filteredRecs],cashWallets:w};
  }));};
