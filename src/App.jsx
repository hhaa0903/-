...
async function scanReceipt(base64Img,apiKey,preferCurrs,allCurrs){
  ...
  const prompt=`你是收據辨識助手，分析圖片並以JSON回傳：
常用幣別（優先）：${currList}
所有幣別：${allCurrList}
回傳格式（只回傳JSON）：
{"amount":數字,"currency":"幣別代碼","note":"品項或店名","detail":"詳細摘要如航班號房型","category":"food/transport/hotel/flight/ticket/shopping/activity/other","platform":"booking/agoda/airbnb/trip/expedia/kkday/klook/google_flights/airline/grab/uber/bolt/indrive/manual","isOverseas":true或false,"confidence":"high/medium/low"}
規則：取合計金額；非台幣通常isOverseas:true；餐廳→food；計程車/叫車→transport；飯店→hotel；機票→flight；KKday/Klook→ticket；百貨→shopping；門票→activity`;
...
function exportCSV(trip,allCurrs,cards,customPays){
  ...
  if((trip.receivables??[]).length){
    ...
    rows.push(["日期","朋友","品項","台幣金額","已還款"].map(esc));
    (trip.receivables??[]).forEach(r=>{rows.push([r.date,r.friend,r.note,Math.round(r.amount),r.paid?"是":"否"].map(esc));});
  }
...
function useStore(){
  ...
  const _adjustWallet=(wallets,currency,delta)=>{const idx=wallets.findIndex(w=>w.currency===currency);if(idx<0)return wallets;const newW=[...wallets];newW[idx]={...newW[idx],amount:Math.max(0,newW[idx].amount+delta)};return newW;};
  ...
  const addExpense=(tid,e,recs=[])=>{setTrips(prev=>prev.map(x=>{if(x.id!==tid)return x;let wallets=[...(x.cashWallets??[])];if(e.payment==="cash")wallets=_adjustWallet(wallets,e.currency,-e.amount);return{...x,expenses:[{...e,id:uid()},...(x.expenses??[])],receivables:[...recs,...(x.receivables??[])],cashWallets:wallets};}));};
  const delExpense=(tid,eid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const exp=(t.expenses??[]).find(e=>e.id===eid);let wallets=[...(t.cashWallets??[])];if(exp&&exp.payment==="cash")wallets=_adjustWallet(wallets,exp.currency,+exp.amount);return{...t,expenses:(t.expenses??[]).filter(e=>e.id!==eid),cashWallets:wallets};}));};
  const editExpense=(tid,eid,patch,newRecs=[])=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const old=(t.expenses??[]).find(e=>e.id===eid);let wallets=[...(t.cashWallets??[])];if(old&&old.payment==="cash")wallets=_adjustWallet(wallets,old.currency,+old.amount);if(patch.payment==="cash")wallets=_adjustWallet(wallets,patch.currency,-patch.amount);const filteredRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);const recsWithId=newRecs.map(r=>({...r,expenseId:eid}));return{...t,expenses:(t.expenses??[]).map(e=>e.id===eid?{...e,...patch,id:eid}:e),receivables:[...recsWithId,...filteredRecs],cashWallets:wallets};}));};
  ...
  const buyWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const w=(t.wishlist??[]).find(x=>x.id===wid);if(!w)return t;const rate=allCurrs.find(c=>c.code===currency)?.r??1;const totalTWD=price*qty*rate;const selCard=cardsArr.find(c=>c.id===cardId);const feeTotal=selCard?totalTWD*(selCard.feeRateOverseas??selCard.feeRate??0)/100:0;const grand=totalTWD+feeTotal;const proxy={id:uid(),buyer:w.buyer,item:w.item,price,qty,currency,payment,cardId,note:w.note,img:w.img,feeTotal,totalTWD:grand,paid:false,date:todayStr()};return{...t,wishlist:(t.wishlist??[]).map(x=>x.id===wid?{...x,done:true}:x),proxies:[proxy,...(t.proxies??[])]};}));};
  ...
  const addExchange=(tid,ex)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;let wallets=[...(t.cashWallets??[])];wallets=_adjustWallet(wallets,ex.fromCurrency,-ex.fromAmount);const toIdx=wallets.findIndex(w=>w.currency===ex.toCurrency);if(toIdx>=0)wallets[toIdx]={...wallets[toIdx],amount:wallets[toIdx].amount+ex.toAmount};else wallets=[...wallets,{id:uid(),currency:ex.toCurrency,amount:ex.toAmount,initialAmount:ex.toAmount}];return{...t,exchanges:[{...ex,id:uid()},...(t.exchanges??[])],cashWallets:wallets};}));};
  const delExchange=(tid,eid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const ex=(t.exchanges??[]).find(e=>e.id===eid);if(!ex)return t;let wallets=[...(t.cashWallets??[])];wallets=_adjustWallet(wallets,ex.fromCurrency,+ex.fromAmount);wallets=_adjustWallet(wallets,ex.toCurrency,-ex.toAmount);return{...t,exchanges:(t.exchanges??[]).filter(e=>e.id!==eid),cashWallets:wallets};}));};
    ...
    <div style={{maxWidth:680,margin:"0 auto",padding:"0 0 60px",background:"#fff"}}>
      ...
      <div style={{padding:"28px 36px 0"}}>
        ...
        {unpaidProxies.length>0&&<div style={{marginTop:24}}>
          ...
          {[...new Set(unpaidProxies.map(p=>p.buyer))].map(buyer=>{
            const items=unpaidProxies.filter(p=>p.buyer===buyer);
            const bTotal=items.reduce((s,p)=>s+(p.totalTWD??0),0);
            return(<div key={buyer} style={{marginBottom:16,background:"#F9F6FF",borderRadius:14,padding:"12px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:700,color:"#5A5A5A"}}>👤 {buyer}</span><span style={{fontSize:14,fontWeight:800,color:"#B5A8C8"}}>NT${fmt(bTotal)}</span></div>
              {mode==="detail"&&items.map(p=>{const curr=allCurrs.find(c=>c.code===p.currency);const sym=curr?.sym??p.currency;return(<div key={p.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#8A8A7A",marginBottom:4,paddingLeft:8}}><span>{p.item} ×{p.qty}　{sym}{fmtD(p.price,0)}</span><span>NT${fmt(p.totalTWD??0)}</span></div>);})}
            </div>);
          })}
        </div>}
