...
const CATS=[{id:"food",e:"🍜",l:"餐飲",c:"#C9A89A"},{id:"transport",e:"🚌",l:"交通",c:"#A8B5A2"},{id:"hotel",e:"🏨",l:"住宿",c:"#B5A8C8"},{id:"flight",e:"✈️",l:"機票",c:"#9AB5B5"},{id:"ticket",e:"🎭",l:"訂票體驗",c:"#C8A8B5"},{id:"shopping",e:"🛍️",l:"購物",c:"#C8B89A"},{id:"activity",e:"🎯",l:"活動",c:"#A8C8B5"},{id:"other",e:"📎",l:"其他",c:"#BBBBA8"}];
const PLATFORMS=[{id:"booking",l:"Booking.com",e:"🏨"},{id:"agoda",l:"Agoda",e:"🏨"},{id:"airbnb",l:"Airbnb",e:"🏠"},{id:"trip",l:"Trip.com",e:"🌐"},{id:"expedia",l:"Expedia",e:"✈️"},{id:"kkday",l:"KKday",e:"🎭"},{id:"klook",l:"Klook",e:"🎡"},{id:"google_flights",l:"Google Flights",e:"✈️"},{id:"airline",l:"航空公司官網",e:"✈️"},{id:"grab",l:"Grab",e:"🚗"},{id:"uber",l:"Uber",e:"🚕"},{id:"bolt",l:"Bolt",e:"⚡"},{id:"indrive",l:"inDrive",e:"🚙"},{id:"manual",l:"手動輸入",e:"✏️"}];
const BASE_CURRS=[
...
const BASE_PAYS=[{id:"cash",e:"💵",l:"現金",feeRateOverseas:0,feeRateDomestic:0,isBuiltin:true},{id:"linepay",e:"📱",l:"Line Pay",feeRateOverseas:0,feeRateDomestic:0,isBuiltin:true}];
...
const fmtD=(n,d=0)=>new Intl.NumberFormat("zh-TW",{maximumFractionDigits:d}).format(n);
...
async function scanReceipt(base64Img,apiKey,preferCurrs,allCurrs){
  ...
  const prompt=`你是專業收據辨識助手。請仔細分析這張收據圖片，提取關鍵資訊並以JSON格式回傳。

這趟旅行的常用幣別（請優先辨識這些）：${currList}
所有支援幣別：${allCurrList}

請回傳以下JSON格式（只回傳JSON，不要其他文字或markdown）：
{
  "amount": 數字（最終合計金額，不要逗號），
  "currency": "幣別代碼如JPY/TWD/THB/USD",
  "note": "店名或品項（保留原文）",
  "detail": "詳細說明如航班號、房型、日期範圍",
  "category": "food或transport或hotel或flight或ticket或shopping或activity或other",
  "platform": "booking或agoda或airbnb或trip或expedia或kkday或klook或google_flights或airline或grab或uber或bolt或indrive或manual",
  "isOverseas": true或false,
  "confidence": "high或medium或low"
}

辨識規則：
- 金額：找「合計」「総計」「Total」「お会計」「รวม」「합계」等關鍵字，取最終金額
- 幣別：從符號判斷（¥/円→JPY，₩/원→KRW，฿/บาท→THB，$→USD，€→EUR，NT$/元→TWD）
- 優先從常用幣別中選擇
- 非台幣消費通常isOverseas為true
- 餐廳食物→food；捷運計程車叫車→transport；飯店旅館→hotel；機票→flight；門票體驗→activity；百貨購物→shopping`;
  const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-opus-4-6",max_tokens:800,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:base64Img.split(",")[1]}},{type:"text",text:prompt}]}]})});
...
function ReceiptScanBtn({apiKey,preferCurrs,allCurrs,onResult}){
  const [scanning,setScanning]=useState(false);const [err,setErr]=useState(null);
  const handleScan=async e=>{
    ...
    catch(ex){setErr(`辨識失敗：${ex.message||"請重試或手動記帳"}`);}
...
function exportCSV(trip,allCurrs,cards,customPays){
  const BOM="\uFEFF";
  const esc=v=>`"${String(v??'').replace(/"/g,'""')}"`;
  const rows=[["日期","類別","簡要品項","詳細摘要","原幣金額","幣別","台幣金額","我的花費","支付方式","信用卡","平台","海外/國內","分攤人數","手續費(台幣)"].map(esc)];
  (trip.expenses??[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(e=>{
    const cat=CATS.find(c=>c.id===e.category)?.l??"其他";
    const curr=allCurrs.find(c=>c.code===e.currency);
    const twdAmt=e.amount*(curr?.r??1);
    const selCard=cards.find(c=>c.id===e.cardId);
    const selCustomPay=customPays?.find(p=>p.id===e.payment);
    const payLabel=e.payment==="cash"?"現金":e.payment==="card"?(selCard?.name??"信用卡"):(selCustomPay?.name??e.payment);
    const platLabel=e.platformLabel||(e.platform&&e.platform!=="manual"?(PLATFORMS.find(p=>p.id===e.platform)?.l??""):"");
    const persons=(e.participants?.length??0)+(e.iParticipate?1:0);
    rows.push([e.date,cat,e.note??"",e.detail??"",e.amount,e.currency,Math.round(twdAmt),Math.round(e.myShare??0),payLabel,selCard?.name??"",platLabel,e.isOverseas===false?"國內":"海外",persons,Math.round(e.feeTotal??0)].map(esc));
  });
  if((trip.proxies??[]).length){
    rows.push([]);rows.push([esc("--- 代購清單 ---"),"","","","","","","","","","","","",""]);
    rows.push(["日期","委託人","品項","數量","原幣單價","幣別","台幣合計(含手續費)","支付方式","備註","已收款"].map(esc));
    (trip.proxies??[]).forEach(p=>{rows.push([p.date??"",p.buyer,p.item,p.qty,p.price,p.currency,Math.round(p.totalTWD??0),p.payment,p.note??"",p.paid?"是":"否"].map(esc));});
  }
  if((trip.receivables??[]).length){
    rows.push([]);rows.push([esc("--- 代墊分攤 ---"),"","","","","","","","","","","","",""]);
    rows.push(["日期","朋友","品項","台幣金額","已還款","還款方式","還款幣別"].map(esc));
    (trip.receivables??[]).forEach(r=>{rows.push([r.date,r.friend,r.note,Math.round(r.amount),r.paid?"是":"否",r.repayType??"",r.repaymentCurrency??""].map(esc));});
  }
  if((trip.exchanges??[]).length){
    rows.push([]);rows.push([esc("--- 換匯紀錄 ---"),"","","","","","","","","","","","",""]);
    rows.push(["日期","支出幣別","支出金額","換得幣別","換得金額","匯率","備註"].map(esc));
    (trip.exchanges??[]).forEach(ex=>{const rate=(+ex.toAmount/+ex.fromAmount).toFixed(4);rows.push([ex.date,ex.fromCurrency,ex.fromAmount,ex.toCurrency,ex.toAmount,rate,ex.note??""].map(esc));});
  }
  if((trip.cashWallets??[]).length){
    rows.push([]);rows.push([esc("--- 現金錢包 ---"),"","","","","","","","","","","","",""]);
    rows.push(["幣別","現在餘額","原始攜帶","台幣換算"].map(esc));
    (trip.cashWallets??[]).forEach(w=>{const r=allCurrs.find(c=>c.code===w.currency)?.r??1;rows.push([w.currency,w.amount,w.initialAmount??w.amount,Math.round(w.amount*r)].map(esc));});
  }
  const csv=BOM+rows.map(r=>Array.isArray(r)?r.join(","):r).join("\n");
  const blob=new Blob([csv],{type:"text/csv;charset=utf-8;"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");
  a.href=url;a.download=`${trip.name}_${todayStr()}.csv`;
  document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
      }function WelcomeModal({onClose}){return(<div style={{position:"fixed",inset:0,background:"rgba(80,70,60,0.45)",backdropFilter:"blur(12px)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}><div style={{width:"100%",maxWidth:360,background:"#FFFFFF",borderRadius:28,padding:"32px 28px",textAlign:"center"}}><div style={{width:56,height:6,borderRadius:99,background:"linear-gradient(90deg,#C9A89A,#B5A8C8,#A8B5A2)",margin:"0 auto 24px"}}/><div style={{fontSize:36,marginBottom:12}}>✈️</div><h2 style={{fontFamily:SERIF,fontSize:20,color:"#5A5A5A",margin:"0 0 8px"}}>旅行足跡帳本</h2><p style={{fontSize:13,color:"#8A8A7A",margin:"0 0 20px"}}>Travel Footprint</p><div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:28,textAlign:"left"}}>{[{icon:"📱",title:"離線完全可用",desc:"資料存於手機本地，飛機上也能記帳"},{icon:"💵",title:"現金管理",desc:"追蹤各幣別現金餘額與換匯紀錄"},{icon:"👥",title:"多人旅行",desc:"自動拆帳、代購、一鍵LINE帳單"},{icon:"📷",title:"拍收據自動記帳",desc:"AI辨識多國語言，自動填入金額"}].map(f=>(<div key={f.icon} style={{display:"flex",gap:12,alignItems:"flex-start",background:"#F9F8F6",borderRadius:14,padding:"12px 14px"}}><span style={{fontSize:20,flexShrink:0}}>{f.icon}</span><div><div style={{fontSize:13,fontWeight:700,color:"#5A5A5A",marginBottom:2}}>{f.title}</div><div style={{fontSize:11,color:"#BBBBA8"}}>{f.desc}</div></div></div>))}</div><button onClick={onClose} style={{width:"100%",padding:"15px",borderRadius:18,border:"none",background:"linear-gradient(135deg,#C9A89A,#D4B8AB)",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:SANS}}>開始使用 →</button></div></div>);}
function useStore(){
  ...
  const [customPays,setCustomPays]=useState(()=>lsGet("tf_pays",[]));
  ...
  useEffect(()=>lsSet("tf_pays",customPays),[customPays]);
  useEffect(()=>lsSet("tf_apikey",apiKey),[apiKey]);
  const allCurrs=[...BASE_CURRS,...customCurrs];
  const allPays=[...BASE_PAYS,...customPays];
  const active=trips.find(t=>t.id===activeId)??null;
  const upd=(id,p)=>setTrips(prev=>prev.map(t=>t.id===id?{...t,...p}:t));
  const _adj=(wallets,currency,delta)=>{const idx=wallets.findIndex(w=>w.currency===currency);if(idx<0)return wallets;const n=[...wallets];n[idx]={...n[idx],amount:Math.max(0,n[idx].amount+delta)};return n;};
  const createTrip=d=>{const t={id:uid(),createdAt:todayStr(),archived:false,expenses:[],proxies:[],wishlist:[],selflist:[],receivables:[],exchanges:[],cashWallets:[],customDays:null,...d};setTrips(p=>[t,...p]);setActiveId(t.id);return t.id;};
  ...
  const setCustomDays=(tid,days)=>upd(tid,{customDays:days});
  const addExpense=(tid,e,recs=[])=>{setTrips(prev=>prev.map(x=>{if(x.id!==tid)return x;const newId=uid();let w=[...(x.cashWallets??[])];if(e.payment==="cash")w=_adj(w,e.currency,-e.amount);const recsWithId=recs.map(r=>({...r,expenseId:newId}));return{...x,expenses:[{...e,id:newId},...(x.expenses??[])],receivables:[...recsWithId,...(x.receivables??[])],cashWallets:w};}));};
  const delExpense=(tid,eid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const exp=(t.expenses??[]).find(e=>e.id===eid);let w=[...(t.cashWallets??[])];if(exp?.payment==="cash")w=_adj(w,exp.currency,+exp.amount);const recsToDelete=(t.receivables??[]).filter(r=>r.expenseId===eid);recsToDelete.forEach(r=>{if(r.paid&&r.repayType==="cash"&&r.repaymentCurrency&&r.repaymentAmount>0)w=_adj(w,r.repaymentCurrency,-r.repaymentAmount);});return{...t,expenses:(t.expenses??[]).filter(e=>e.id!==eid),receivables:(t.receivables??[]).filter(r=>r.expenseId!==eid),cashWallets:w};}));};
  const editExpense=(tid,eid,patch,newRecs=[])=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const old=(t.expenses??[]).find(e=>e.id===eid);let w=[...(t.cashWallets??[])];if(old?.payment==="cash")w=_adj(w,old.currency,+old.amount);if(patch.payment==="cash")w=_adj(w,patch.currency,-patch.amount);const paidRecs=(t.receivables??[]).filter(r=>r.expenseId===eid&&r.paid);const otherRecs=(t.receivables??[]).filter(r=>r.expenseId!==eid);const paidFriends=new Set(paidRecs.map(r=>r.friend));const recsWithId=newRecs.filter(r=>!paidFriends.has(r.friend)).map(r=>({...r,expenseId:eid}));return{...t,expenses:(t.expenses??[]).map(e=>e.id===eid?{...e,...patch,id:eid}:e),receivables:[...recsWithId,...paidRecs,...otherRecs],cashWallets:w};}));};
  ...
  const buyWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const w=(t.wishlist??[]).find(x=>x.id===wid);if(!w)return t;const rate=allCurrs.find(c=>c.code===currency)?.r??1;const totalTWD=price*qty*rate;const selCard=cardsArr.find(c=>c.id===cardId);const feeTotal=selCard?totalTWD*(selCard.feeRateOverseas??selCard.feeRate??0)/100:0;const grand=totalTWD+feeTotal;const proxy={id:uid(),buyer:w.buyer,item:w.item,price,qty,currency,payment,cardId,note:w.note,img:w.img,feeTotal,totalTWD:grand,paid:false,date:todayStr()};let wallets=[...(t.cashWallets??[])];if(payment==="cash")wallets=_adj(wallets,currency,-price*qty);return{...t,wishlist:(t.wishlist??[]).map(x=>x.id===wid?{...x,done:true}:x),proxies:[proxy,...(t.proxies??[])],cashWallets:wallets};}));};
  ...
  const buySelfWish=(tid,wid,price,qty,currency,payment,cardId,cardsArr)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const w=(t.selflist??[]).find(x=>x.id===wid);if(!w)return t;const rate=allCurrs.find(c=>c.code===currency)?.r??1;const totalTWD=price*qty*rate;const selCard=cardsArr.find(c=>c.id===cardId);const feeTotal=selCard?totalTWD*(selCard.feeRateOverseas??selCard.feeRate??0)/100:0;const myShare=totalTWD+feeTotal;const ne={id:uid(),date:todayStr(),category:"shopping",note:w.item,amount:price*qty,currency,payment,cardId,iParticipate:true,participants:[],myShare,friendShares:{},feeTotal,isOverseas:true};let wallets=[...(t.cashWallets??[])];if(payment==="cash")wallets=_adj(wallets,currency,-price*qty);return{...t,selflist:(t.selflist??[]).map(x=>x.id===wid?{...x,done:true}:x),expenses:[ne,...(t.expenses??[])],cashWallets:wallets};}));};
  const addProxy=(tid,p)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;let wallets=[...(t.cashWallets??[])];if(p.payment==="cash")wallets=_adj(wallets,p.currency,-p.price*p.qty);return{...t,proxies:[{...p,id:uid(),paid:false},...(t.proxies??[])],cashWallets:wallets};}));};
  const delProxy=(tid,pid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const proxy=(t.proxies??[]).find(p=>p.id===pid);let wallets=[...(t.cashWallets??[])];if(proxy?.payment==="cash")wallets=_adj(wallets,proxy.currency,proxy.price*proxy.qty);return{...t,proxies:(t.proxies??[]).filter(p=>p.id!==pid),cashWallets:wallets};}));};
  const toggleProxy=(tid,pid)=>{setTrips(prev=>prev.map(t=>t.id===tid?{...t,proxies:(t.proxies??[]).map(p=>p.id===pid?{...p,paid:!p.paid}:p)}:t));};
  const markManyPaid=(tid,rids,repayType,repaymentCurrency,repaymentAmount)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;let wallets=[...(t.cashWallets??[])];if(repayType==="cash"&&repaymentCurrency&&repaymentAmount>0){const toIdx=wallets.findIndex(w=>w.currency===repaymentCurrency);if(toIdx>=0)wallets=_adj(wallets,repaymentCurrency,repaymentAmount);else wallets=[...wallets,{id:uid(),currency:repaymentCurrency,amount:repaymentAmount,initialAmount:0}];}const newRecs=(t.receivables??[]).map(r=>rids.includes(r.id)?{...r,paid:true,repayType:repayType??"transfer",repaymentCurrency:repaymentCurrency??"",repaymentAmount:repaymentAmount??0}:r);return{...t,receivables:newRecs,cashWallets:wallets};}));};
  const unmarkPaid=(tid,rid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const rec=(t.receivables??[]).find(r=>r.id===rid);let wallets=[...(t.cashWallets??[])];if(rec?.paid&&rec?.repayType==="cash"&&rec?.repaymentCurrency&&rec?.repaymentAmount>0)wallets=_adj(wallets,rec.repaymentCurrency,-rec.repaymentAmount);return{...t,receivables:(t.receivables??[]).map(r=>r.id===rid?{...r,paid:false,repayType:null,repaymentCurrency:null,repaymentAmount:0}:r),cashWallets:wallets};}));};
  const delRec=(tid,rid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const rec=(t.receivables??[]).find(r=>r.id===rid);let wallets=[...(t.cashWallets??[])];if(rec?.paid&&rec?.repayType==="cash"&&rec?.repaymentCurrency&&rec?.repaymentAmount>0)wallets=_adj(wallets,rec.repaymentCurrency,-rec.repaymentAmount);return{...t,receivables:(t.receivables??[]).filter(r=>r.id!==rid),cashWallets:wallets};}));};
  ...
  const addCashWallet=(tid,wallet)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const existing=(t.cashWallets??[]).findIndex(w=>w.currency===wallet.currency);if(existing>=0){const nw=[...(t.cashWallets??[])];nw[existing]={...nw[existing],amount:nw[existing].amount+wallet.amount,initialAmount:(nw[existing].initialAmount??nw[existing].amount)+wallet.amount};return{...t,cashWallets:nw};}return{...t,cashWallets:[...(t.cashWallets??[]),{...wallet,id:uid(),initialAmount:wallet.amount}]};}));};
  const updateCashWallet=(tid,wid,amount)=>{setTrips(prev=>prev.map(t=>t.id===tid?{...t,cashWallets:(t.cashWallets??[]).map(w=>w.id===wid?{...w,amount}:w)}:t));};
  const delCashWallet=(tid,wid)=>{setTrips(prev=>prev.map(t=>t.id===tid?{...t,cashWallets:(t.cashWallets??[]).filter(w=>w.id!==wid)}:t));};
  const addExchange=(tid,ex)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;let w=[...(t.cashWallets??[])];w=_adj(w,ex.fromCurrency,-ex.fromAmount);const toIdx=w.findIndex(x=>x.currency===ex.toCurrency);if(toIdx>=0)w[toIdx]={...w[toIdx],amount:w[toIdx].amount+ex.toAmount};else w=[...w,{id:uid(),currency:ex.toCurrency,amount:ex.toAmount,initialAmount:ex.toAmount}];return{...t,exchanges:[{...ex,id:uid()},...(t.exchanges??[])],cashWallets:w};}));};
  const delExchange=(tid,eid)=>{setTrips(prev=>prev.map(t=>{if(t.id!==tid)return t;const ex=(t.exchanges??[]).find(e=>e.id===eid);if(!ex)return t;let w=[...(t.cashWallets??[])];w=_adj(w,ex.fromCurrency,+ex.fromAmount);w=_adj(w,ex.toCurrency,-ex.toAmount);return{...t,exchanges:(t.exchanges??[]).filter(e=>e.id!==eid),cashWallets:w};}));};
  const restoreAll=data=>{if(data.trips)setTrips(data.trips);if(data.banks)setBanks(data.banks);if(data.cards)setCards(data.cards);if(data.friends)setFriends(data.friends);if(data.customCurrs)setCustomCurrs(data.customCurrs);if(data.customPays)setCustomPays(data.customPays);if(data.trips?.length>0)setActiveId(data.trips[0].id);};
  return{trips,activeId,setActiveId,active,banks,setBanks,cards,setCards,friends,setFriends,customCurrs,setCustomCurrs,customPays,setCustomPays,allCurrs,allPays,apiKey,setApiKey,createTrip,archiveTrip,reopenTrip,deleteTrip,setCustomDays,addExpense,delExpense,editExpense,addWish,delWish,buyWish,addSelfWish,delSelfWish,buySelfWish,addProxy,delProxy,toggleProxy,markManyPaid,unmarkPaid,delRec,markManyProxyPaid,convertToExp,addCashWallet,updateCashWallet,delCashWallet,addExchange,delExchange,upd,restoreAll};
    }function makeBill(friend,trip,banks,bankId,mode,allCurrs){
  ...
  const pTotal=prox.reduce((s,p)=>s+(p.totalTWD??0),0);
  const grand=rTotal+pTotal;
  const bank=banks.find(b=>b.id===bankId);
  const lines=[`📋 ${trip.name}｜${friend} 的帳單`,"━━━━━━━━━━━━━━━━━━━━"];
  if(recs.length){lines.push("\n💸 代墊分攤");if(mode==="detail")recs.forEach(r=>{const curr=allCurrs.find(c=>c.code===r.currency);const origStr=(r.currency&&r.currency!=="TWD"&&r.origAmount)?` (${curr?.sym??r.currency}${fmtD(r.origAmount,0)})`:"";lines.push(`  ${r.note}${origStr}　NT$${fmt(r.amount)}`);});else lines.push(`  共${recs.length}筆　NT$${fmt(rTotal)}`);}
  if(prox.length){lines.push("\n🛍️ 代購清單");if(mode==="detail")prox.forEach(p=>{const curr=allCurrs.find(c=>c.code===p.currency);const sym=curr?.sym??p.currency;lines.push(`  ${p.item} ×${p.qty}　${sym}${fmtD(p.price,0)} → NT$${fmt(p.totalTWD??0)}`);});else lines.push(`  共${prox.length}件　NT$${fmt(pTotal)}`);}
  lines.push("\n━━━━━━━━━━━━━━━━━━━━",`💰 合計應付　NT$${fmt(grand)}`);
  if(bank)lines.push("\n匯款 🏦",`  ${bank.bankName}`,`  帳號：${bank.account}`,`  戶名：${bank.holder}`);
  lines.push("\n麻煩轉帳給我，謝謝 🙏");
  return lines.join("\n");
}
function SettingsSheet({banks,setBanks,cards,setCards,friends,setFriends,trips,customCurrs,setCustomCurrs,customPays,setCustomPays,apiKey,setApiKey,onRestore,onClose}){
  const [tab,setTab]=useState("friends");
  const [nf,setNf]=useState("");const [nb,setNb]=useState({bankName:"",account:"",holder:""});const [nc,setNc]=useState({name:"",feeRateOverseas:"",feeRateDomestic:"0"});const [np,setNp]=useState({name:"",e:"💳",feeRateOverseas:"",feeRateDomestic:"0"});
  const [restoreMsg,setRestoreMsg]=useState(null);const [newCurr,setNewCurr]=useState({code:"",sym:"",l:"",r:""});const [fetchingRate,setFetchingRate]=useState(false);const [keyInput,setKeyInput]=useState(apiKey);const [keyVisible,setKeyVisible]=useState(false);
  ...
  const addPay=()=>{if(np.name.trim()&&np.feeRateOverseas!==""){setCustomPays(p=>[...p,{...np,id:uid(),feeRateOverseas:+np.feeRateOverseas,feeRateDomestic:+np.feeRateDomestic}]);setNp({name:"",e:"💳",feeRateOverseas:"",feeRateDomestic:"0"});}};
  ...
  const handleBackup=()=>{const data={version:1,exportedAt:new Date().toISOString(),trips,banks,cards,friends,customCurrs,customPays};const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`旅行帳本備份_${new Date().toLocaleDateString("zh-TW").replace(/\//g,"-")}.json`;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);};
  ...
  return(<Sheet onClose={onClose}><SHead title="⚙️ 設定" onClose={onClose}/>
    <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>{[{id:"friends",l:"👥 朋友"},{id:"banks",l:"🏦 帳號"},{id:"cards",l:"💳 信用卡"},{id:"pays",l:"💸 支付方式"},{id:"currs",l:"💱 幣別"},{id:"ai",l:"🤖 AI辨識"},{id:"backup",l:"💾 備份"}].map(t=><Pill key={t.id} active={tab===t.id} color="#C9A89A" onClick={()=>setTab(t.id)}>{t.l}</Pill>)}</div>
    ...
    {tab==="cards"&&<><Lbl ch="我的信用卡"/><div style={{background:"#EAE6F2",borderRadius:14,padding:"10px 14px",marginBottom:12,fontSize:12,color:"#8A78A8",lineHeight:1.7}}>💡 海外消費才計算手續費，國內消費手續費通常為 0%</div><Inp placeholder="卡片名稱" value={nc.name} onChange={e=>setNc(p=>({...p,name:e.target.value}))} style={{marginBottom:8}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}><div><Lbl ch="海外手續費 %"/><Inp placeholder="1.5" type="number" value={nc.feeRateOverseas} onChange={e=>setNc(p=>({...p,feeRateOverseas:e.target.value}))}/></div><div><Lbl ch="國內手續費 %"/><Inp placeholder="0" type="number" value={nc.feeRateDomestic} onChange={e=>setNc(p=>({...p,feeRateDomestic:e.target.value}))}/></div></div><Btn onClick={addCard} style={{width:"100%",marginBottom:16}}>+ 新增信用卡</Btn>{cards.map(c=><div key={c.id} style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:14,fontWeight:600,color:"#5A5A5A"}}>💳 {c.name}</div><div style={{fontSize:12,color:"#8A8A7A"}}>海外 {c.feeRateOverseas??c.feeRate??0}%・國內 {c.feeRateDomestic??0}%</div></div><button onClick={()=>setCards(p=>p.filter(x=>x.id!==c.id))} style={{background:"none",border:"none",color:"#BBBBA8",cursor:"pointer",fontSize:18}}>🗑</button></div>)}</>}
    {tab==="pays"&&<><Lbl ch="自訂支付方式"/><div style={{background:"#E4EBE2",borderRadius:14,padding:"10px 14px",marginBottom:12,fontSize:12,color:"#6A8A7A",lineHeight:1.7}}>💡 新增 Apple Pay、全支付等行動支付，可設定海外與國內手續費</div><div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>{EMOJI_OPTS.map(e=><button key={e} onClick={()=>setNp(p=>({...p,e}))} style={{width:36,height:36,borderRadius:10,border:`1.5px solid ${np.e===e?"#C9A89A":"#EDEBE6"}`,background:np.e===e?"#F0E6E1":"transparent",fontSize:18,cursor:"pointer"}}>{e}</button>)}</div><Inp placeholder="支付方式名稱（如 Apple Pay）" value={np.name} onChange={e=>setNp(p=>({...p,name:e.target.value}))} style={{marginBottom:8}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}><div><Lbl ch="海外手續費 %"/><Inp placeholder="1.5" type="number" value={np.feeRateOverseas} onChange={e=>setNp(p=>({...p,feeRateOverseas:e.target.value}))}/></div><div><Lbl ch="國內手續費 %"/><Inp placeholder="0" type="number" value={np.feeRateDomestic} onChange={e=>setNp(p=>({...p,feeRateDomestic:e.target.value}))}/></div></div><Btn onClick={addPay} style={{width:"100%",marginBottom:16}}>+ 新增支付方式</Btn>{customPays.map(p=><div key={p.id} style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:14,fontWeight:600,color:"#5A5A5A"}}>{p.e} {p.name}</div><div style={{fontSize:12,color:"#8A8A7A"}}>海外 {p.feeRateOverseas}%・國內 {p.feeRateDomestic}%</div></div><button onClick={()=>setCustomPays(prev=>prev.filter(x=>x.id!==p.id))} style={{background:"none",border:"none",color:"#BBBBA8",cursor:"pointer",fontSize:18}}>🗑</button></div>)}{customPays.length===0&&<div style={{textAlign:"center",padding:"20px 0",color:"#BBBBA8",fontSize:13}}>還沒有自訂支付方式</div>}</>}
    ...
    {tab==="ai"&&<><div style={{background:"#EAE6F2",borderRadius:14,padding:"12px 14px",marginBottom:16,fontSize:12,color:"#8A78A8",lineHeight:1.8}}>🤖 使用 Anthropic Claude AI 辨識收據<br/>支援日文、韓文、泰文、英文等多國語言<br/>使用 claude-opus-4-6 模型，辨識更準確</div><Lbl ch="Anthropic API Key"/><div style={{display:"flex",gap:8,marginBottom:8}}><Inp type={keyVisible?"text":"password"} placeholder="sk-ant-..." value={keyInput} onChange={e=>setKeyInput(e.target.value)} style={{flex:1,fontFamily:"monospace",fontSize:12}}/><button onClick={()=>setKeyVisible(p=>!p)} style={{padding:"12px",borderRadius:14,border:"1.5px solid #EDEBE6",background:"#FDFCFB",cursor:"pointer",fontSize:16}}>{keyVisible?"🙈":"👁"}</button></div><Btn onClick={()=>setApiKey(keyInput)} color="#B5A8C8" style={{width:"100%",marginBottom:12}}>💾 儲存 API Key</Btn>{apiKey&&<div style={{background:"#E2EFE8",borderRadius:12,padding:"10px 14px",fontSize:12,color:"#88B098",fontWeight:600}}>✅ API Key 已設定</div>}<div style={{background:"#FFF8E8",border:"1px solid #F0D88A",borderRadius:14,padding:"12px 14px",marginTop:12,fontSize:11,color:"#8A7040",lineHeight:1.7}}>⚠️ API Key 只存在你的手機，不會上傳任何伺服器<br/>取得方式：前往 console.anthropic.com 申請</div></>}
  ...
  return(<Sheet onClose={onClose}><SHead title="建立新旅行" onClose={onClose}/>
    ...
    <Lbl ch="這趟旅行常用幣別"/><div style={{fontSize:11,color:"#BBBBA8",marginBottom:10}}>記帳時優先顯示，AI辨識收據時也會優先辨識</div>
...
function PlatformSel({value,manualVal,onChange,onManualChange}){
  return(<div>
    <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:8}}>{PLATFORMS.map(p=><button key={p.id} onClick={()=>onChange(p.id)} style={{padding:"6px 10px",borderRadius:99,border:`1.5px solid ${value===p.id?"#9AB5B5":"#EDEBE6"}`,background:value===p.id?"#E8F0F0":"transparent",color:value===p.id?"#9AB5B5":"#8A8A7A",fontSize:11,fontWeight:value===p.id?700:400,cursor:"pointer",fontFamily:SANS,whiteSpace:"nowrap"}}>{p.e} {p.l}</button>)}</div>
    {value==="manual"&&<Inp placeholder="輸入平台名稱" value={manualVal} onChange={e=>onManualChange(e.target.value)} style={{marginTop:4}}/>}
  </div>);
}
function PaymentSel({value,cardId,onChange,onCardChange,cards,customPays,isOverseas,amtTWD}){
  const [showOther,setShowOther]=useState(false);
  const selCard=cards.find(c=>c.id===cardId);
  const feeRate=selCard?(isOverseas?(selCard.feeRateOverseas??selCard.feeRate??0):(selCard.feeRateDomestic??0)):0;
  const feeTotal=selCard&&amtTWD?amtTWD*feeRate/100:0;
  const selCustomPay=customPays.find(p=>p.id===value);
  const customFeeRate=selCustomPay?(isOverseas?(selCustomPay.feeRateOverseas??0):(selCustomPay.feeRateDomestic??0)):0;
  const customFeeTotal=selCustomPay&&amtTWD?amtTWD*customFeeRate/100:0;
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
      <button onClick={()=>onChange("cash")} style={{padding:"14px",borderRadius:16,border:`2px solid ${value==="cash"?"#C9A89A":"#EDEBE6"}`,background:value==="cash"?"#F0E6E1":"#FDFCFB",color:value==="cash"?"#C9A89A":"#8A8A7A",fontSize:14,fontWeight:value==="cash"?700:400,cursor:"pointer",fontFamily:SANS,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>💵 現金</button>
      <button onClick={()=>onChange("card")} style={{padding:"14px",borderRadius:16,border:`2px solid ${value==="card"?"#B5A8C8":"#EDEBE6"}`,background:value==="card"?"#EAE6F2":"#FDFCFB",color:value==="card"?"#B5A8C8":"#8A8A7A",fontSize:14,fontWeight:value==="card"?700:400,cursor:"pointer",fontFamily:SANS,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>💳 信用卡</button>
    </div>
    {value==="card"&&<div style={{marginBottom:10}}>{cards.length===0?<div style={{fontSize:12,color:"#BBBBA8",padding:"8px 0"}}>請先在設定新增信用卡</div>:<div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:6}}>{cards.map(c=><Pill key={c.id} active={cardId===c.id} color="#B5A8C8" onClick={()=>onCardChange(c.id)}>💳 {c.name}</Pill>)}</div>}{selCard&&amtTWD>0&&<div style={{background:"#EDE6D8",borderRadius:12,padding:"8px 14px",fontSize:12,color:"#C8B89A",fontWeight:600}}>{isOverseas?"海外":"國內"}手續費 {feeRate}%：NT${fmt(feeTotal)}</div>}</div>}
    {customPays.length>0&&<div style={{marginBottom:8}}><button onClick={()=>setShowOther(p=>!p)} style={{background:"none",border:"none",fontSize:12,color:"#A8B5A2",cursor:"pointer",fontFamily:SANS,padding:"4px 0"}}>{showOther?"▲ 收起其他支付":"▼ 其他支付方式"}</button>{showOther&&<div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:8}}>{customPays.map(p=><Pill key={p.id} active={value===p.id} color="#A8B5A2" onClick={()=>onChange(p.id)}>{p.e} {p.name}</Pill>)}</div>}{selCustomPay&&amtTWD>0&&customFeeRate>0&&<div style={{background:"#E4EBE2",borderRadius:12,padding:"8px 14px",fontSize:12,color:"#A8B5A2",fontWeight:600,marginTop:6}}>{isOverseas?"海外":"國內"}手續費 {customFeeRate}%：NT${fmt(customFeeTotal)}</div>}</div>}
  </div>);
}
function AddWishSheet({trip,friends,allCurrs,onSave,onClose}){
  ...
  return(<Sheet onClose={onClose}><SHead title="新增代購清單" onClose={onClose}/>
    ...
    <Lbl ch="備註"/><Inp placeholder="口味、顏色、規格⋯" value={f.note} onChange={e=>set("note",e.target.value)} style={{marginBottom:14}}/>
...
function AddSelfWishSheet({trip,allCurrs,onSave,onClose}){
  ...
  return(<Sheet onClose={onClose}><SHead title="新增自用清單" onClose={onClose}/>
    ...
    <Lbl ch="預計幣別"/><CurrSel value={f.currency} onChange={e=>set("currency",e.target.value)} allCurrs={allCurrs} preferCurrs={preferCurrs}/><div style={{marginBottom:14}}/>
    <Lbl ch="備註"/><Inp placeholder="規格、顏色⋯" value={f.note} onChange={e=>set("note",e.target.value)} style={{marginBottom:14}}/>
    <Lbl ch="參考圖片（選填）"/><ImgUpload img={f.img} onImg={v=>set("img",v)}/>
    <Btn onClick={()=>{if(!f.item.trim())return;onSave(f);onClose();}} color="#9AB5B5" style={{width:"100%"}}>加入自用清單</Btn>
  </Sheet>);
}
function BuySelfWishSheet({wish,trip,cards,allCurrs,customPays,onBuy,onClose}){
  const preferCurrs=trip.preferCurrs??["TWD"];
  ...
  return(<Sheet onClose={onClose}><SHead title="✓ 買了！記入支出" onClose={onClose}/>
    ...
    <Lbl ch="支付方式" s={{marginBottom:10}}/>
    <PaymentSel value={payment} cardId={cardId} onChange={setPayment} onCardChange={setCardId} cards={cards} customPays={customPays} isOverseas={true} amtTWD={totalTWD}/>
...
function BuyWishSheet({wish,trip,cards,allCurrs,customPays,onBuy,onClose}){
  const preferCurrs=trip.preferCurrs??["TWD"];
  const initCurrency=wish.currency||(preferCurrs[0]??"TWD");
  const [price,setPrice]=useState("");const [qty,setQty]=useState("1");const [currency,setCurrency]=useState(initCurrency);const [payment,setPayment]=useState("cash");const [cardId,setCardId]=useState("");
  const totalTWD=price&&!isNaN(price)?+price*+qty*(allCurrs.find(c=>c.code===currency)?.r??1):0;
...
function AddExpenseSheet({trip,friends,cards,allCurrs,customPays,onSave,onClose,prefill,apiKey}){
  const preferCurrs=trip.preferCurrs??["TWD"];
  const [f,setF]=useState({date:todayStr(),category:(prefill?.category??"food"),note:(prefill?.note??""),detail:(prefill?.detail??""),amount:(prefill?.amount??""),currency:(prefill?.currency??(preferCurrs[0]??"TWD")),payment:"cash",cardId:"",iParticipate:true,participants:[],isOverseas:(prefill?.isOverseas??true),platform:(prefill?.platform??""),platformManual:""});
  ...
  const selCustomPay=customPays.find(p=>p.id===f.payment);
  const feeRateCard=selCard?(f.isOverseas?(selCard.feeRateOverseas??selCard.feeRate??0):(selCard.feeRateDomestic??0)):0;
  const feeRatePay=selCustomPay?(f.isOverseas?(selCustomPay.feeRateOverseas??0):(selCustomPay.feeRateDomestic??0)):0;
  const feeRate=f.payment==="card"?feeRateCard:feeRatePay;
  const total=f.participants.length+(f.iParticipate?1:0);
  const base=total>0?amtTWD/total:amtTWD;
  const feeTotal=amtTWD*feeRate/100;const feeEach=total>0?feeTotal/total:0;const myShare=f.iParticipate?base+feeEach:0;
  ...
  const save=()=>{
    ...
    const cardName=selCard?.name??"";const payName=selCustomPay?.name??"";
    const exp={...f,amount:+f.amount,myShare:f.iParticipate?myShare:0,friendShares,feeTotal,platformLabel,cardName,payName};
    const recs=f.participants.map(fr=>({id:uid(),friend:fr,date:f.date,note:f.note||(CATS.find(c=>c.id===f.category)?.l??""),amount:friendShares[fr],paid:false,currency:f.currency,origAmount:+f.amount/((f.participants.length+(f.iParticipate?1:0))||1)}));
  ...
  const handleScanResult=result=>{
    ...
    if(result.note)set("note",result.note);if(result.detail)set("detail",result.detail);
    if(result.category)set("category",result.category);if(result.platform)set("platform",result.platform);
    if(result.isOverseas!==undefined)set("isOverseas",result.isOverseas);setScanDone(true);
  };
  return(<Sheet onClose={onClose}><SHead title="新增支出" onClose={onClose}/>
    ...
    <Lbl ch="支付方式" s={{marginBottom:10}}/>
    <PaymentSel value={f.payment} cardId={f.cardId} onChange={v=>set("payment",v)} onCardChange={v=>set("cardId",v)} cards={cards} customPays={customPays} isOverseas={f.isOverseas} amtTWD={amtTWD}/>
    <div style={{marginBottom:14,marginTop:4}}><Toggle on={f.iParticipate} onChange={v=>set("iParticipate",v)} label="我也有參與這筆消費" sub="關閉則我不分攤"/></div>
...
function EditExpenseSheet({expense,trip,friends,cards,allCurrs,customPays,onSave,onClose}){
  const preferCurrs=trip.preferCurrs??["TWD"];const e=expense;
  const [f,setF]=useState({date:e.date,category:e.category,note:(e.note||""),detail:(e.detail||""),amount:String(e.amount),currency:e.currency,payment:e.payment,cardId:(e.cardId||""),iParticipate:(e.iParticipate??true),participants:(e.participants||[]),isOverseas:(e.isOverseas??true),platform:(e.platform||""),platformManual:(e.platformManual||"")});
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  const toggleP=fr=>set("participants",f.participants.includes(fr)?f.participants.filter(x=>x!==fr):[...f.participants,fr]);
  const amtTWD=f.amount&&!isNaN(f.amount)?+f.amount*(allCurrs.find(c=>c.code===f.currency)?.r??1):0;
  const selCard=cards.find(c=>c.id===f.cardId);const selCustomPay=customPays.find(p=>p.id===f.payment);
  const feeRateCard=selCard?(f.isOverseas?(selCard.feeRateOverseas??selCard.feeRate??0):(selCard.feeRateDomestic??0)):0;
  const feeRatePay=selCustomPay?(f.isOverseas?(selCustomPay.feeRateOverseas??0):(selCustomPay.feeRateDomestic??0)):0;
  const feeRate=f.payment==="card"?feeRateCard:feeRatePay;
  const total=f.participants.length+(f.iParticipate?1:0);const base=total>0?amtTWD/total:amtTWD;
  const feeTotal=amtTWD*feeRate/100;const feeEach=total>0?feeTotal/total:0;const myShare=f.iParticipate?base+feeEach:0;
  const friendShares={};f.participants.forEach(fr=>{friendShares[fr]=base+feeEach;});
  const save=()=>{
    if(!f.amount||isNaN(f.amount)||+f.amount<=0)return;
    const platformLabel=f.platform==="manual"?f.platformManual:(PLATFORMS.find(p=>p.id===f.platform)?.l??"");
    const cardName=selCard?.name??"";const payName=selCustomPay?.name??"";
    const patch={...f,amount:+f.amount,myShare:f.iParticipate?myShare:0,friendShares,feeTotal,platformLabel,cardName,payName};
    const recs=f.participants.map(fr=>({id:uid(),friend:fr,date:f.date,note:f.note||(CATS.find(c=>c.id===f.category)?.l??""),amount:friendShares[fr],paid:false,currency:f.currency,origAmount:+f.amount/((f.participants.length+(f.iParticipate?1:0))||1)}));
  ...
  return(<Sheet onClose={onClose}><SHead title="✏️ 編輯支出" onClose={onClose}/>
    ...
    <Btn onClick={save} color="#A8B5A2" style={{width:"100%"}}>✓ 儲存修改</Btn>
                                                                                                          ...
                                                                                                          }function AddProxySheet({friends,cards,allCurrs,customPays,trip,onSave,onClose}){
  const preferCurrs=trip.preferCurrs??["TWD"];
  ...
  return(<Sheet onClose={onClose}><SHead title="新增代購" onClose={onClose}/>
    ...
    <Lbl ch="支付方式" s={{marginBottom:10}}/>
    <PaymentSel value={f.payment} cardId={f.cardId} onChange={v=>set("payment",v)} onCardChange={v=>set("cardId",v)} cards={cards} customPays={customPays} isOverseas={true} amtTWD={totalTWD}/>
    <Lbl ch="備註" s={{marginTop:10}}/><Inp placeholder="口味、顏色⋯" value={f.note} onChange={e=>set("note",e.target.value)} style={{marginBottom:14}}/>
    <Lbl ch="參考圖片（選填）"/><ImgUpload img={f.img} onImg={v=>set("img",v)}/>
...
function TripList({trips,activeId,onSelect,onCreate,onArchive,onReopen,onDelete,onSettings,allCurrs}){
  ...
  return(<div style={{padding:"0 16px 100px"}}>
    ...
    {active.length>0&&<><div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:12}}>進行中</div>
      {active.map(t=>{
        ...
        const pend=(t.receivables??[]).filter(r=>!r.paid).reduce((s,r)=>s+r.amount,0)+((t.proxies??[]).filter(p=>!p.paid).reduce((s,p)=>s+(p.totalTWD??0),0));
        const wishPending=(t.wishlist??[]).filter(w=>!w.done).length;const selfPending=(t.selflist??[]).filter(w=>!w.done).length;
        const wallets=t.cashWallets??[];
        const walletStr=wallets.filter(w=>w.amount>0).map(w=>{const curr=allCurrs.find(c=>c.code===w.currency);return`${curr?.sym??w.currency}${Math.round(w.amount).toLocaleString()}`;}).join(" · ");
          ...
          <div style={{padding:"14px 16px"}}>
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              ...
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:18,fontWeight:800,color:"#C9A89A"}}>NT${fmt(myT)}</div>
                <div style={{fontSize:10,color:"#BBBBA8"}}>我的花費</div>
                {pend>0&&<div style={{fontSize:11,color:"#88B098",fontWeight:600}}>待收 NT${fmt(pend)}</div>}
                {wishPending>0&&<div style={{fontSize:11,color:"#B5A8C8",fontWeight:600}}>代購 {wishPending} 件</div>}
                {selfPending>0&&<div style={{fontSize:11,color:"#9AB5B5",fontWeight:600}}>自購 {selfPending} 件</div>}
                {walletStr&&<div style={{fontSize:10,color:"#C8B89A",fontWeight:600,marginTop:2}}>💵 {walletStr}</div>}
              </div>
            </div>
    ...
    {trips.length===0&&<div style={{textAlign:"center",padding:"60px 0",color:"#BBBBA8"}}><div style={{fontSize:52,marginBottom:16}}>✈️</div><div style={{fontFamily:SERIF,fontSize:18,color:"#8A8A7A",marginBottom:8}}>還沒有旅行記錄</div><div style={{fontSize:13}}>建立第一趟旅行，開始記帳吧</div></div>}
                ...
                }const DTABS=[{id:"ledger",e:"📖",l:"帳本"},{id:"collect",e:"💰",l:"代墊"},{id:"proxy",e:"🛍️",l:"代購"},{id:"cash",e:"💵",l:"現金"},{id:"stats",e:"📊",l:"統計"}];
  ...
  return(<div style={{minHeight:"100vh",background:"#F9F8F6"}}>
    <div style={{background:cv.g,padding:"44px 20px 28px",position:"relative"}}>
      ...
      <div style={{position:"absolute",bottom:-22,left:16,right:16,background:"#FFFFFF",borderRadius:20,padding:5,display:"flex",boxShadow:"0 4px 20px rgba(150,120,100,0.18)",border:"1px solid #EDEBE6"}}>
        {DTABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"8px 2px",borderRadius:14,border:"none",background:tab===t.id?cv.g:"transparent",color:tab===t.id?"#fff":"#BBBBA8",fontSize:10,fontWeight:tab===t.id?700:400,cursor:"pointer",fontFamily:SANS}}>{t.e}<br/>{t.l}</button>)}
      </div>
    </div>
    <div style={{height:40}}/>
    {tab==="ledger"&&<LedgerTab trip={trip} allCurrs={allCurrs} cards={cards} customPays={store.customPays} onAdd={()=>openExp(null)} onAddWithScan={prefill=>openExp(prefill)} onDel={eid=>store.delExpense(trip.id,eid)} onEdit={e=>setEditingExp(e)} apiKey={store.apiKey} preferCurrs={trip.preferCurrs??["TWD"]}/>}
    {tab==="collect"&&<CollectTab trip={trip} friends={friends} allCurrs={allCurrs} onFriendsChange={store.setFriends} onMarkManyPaid={(rids,type,curr,amt)=>store.markManyPaid(trip.id,rids,type,curr,amt)} onUnmarkPaid={rid=>store.unmarkPaid(trip.id,rid)} onDelRec={rid=>store.delRec(trip.id,rid)} onConvert={rid=>store.convertToExp(trip.id,rid)} onBill={f=>setBillFriend(f)}/>}
    ...
    {tab==="cash"&&<CashTab trip={trip} allCurrs={allCurrs} onAddWallet={w=>store.addCashWallet(trip.id,w)} onUpdateWallet={(wid,amt)=>store.updateCashWallet(trip.id,wid,amt)} onDelWallet={wid=>store.delCashWallet(trip.id,wid)} onAddExchange={ex=>store.addExchange(trip.id,ex)} onDelExchange={eid=>store.delExchange(trip.id,eid)}/>}
    {tab==="stats"&&<StatsTab trip={trip} allCurrs={allCurrs} cards={cards} customPays={store.customPays} onOpenPrint={m=>setPrintMode(m)} onExportCSV={()=>exportCSV(trip,allCurrs,cards,store.customPays)} onSetCustomDays={d=>store.setCustomDays(trip.id,d)}/>}
    {modal==="exp"&&<AddExpenseSheet trip={trip} friends={friends} cards={cards} allCurrs={allCurrs} customPays={store.customPays} onSave={(exp,recs)=>store.addExpense(trip.id,exp,recs)} onClose={()=>{setModal(null);setExpPrefill(null);}} prefill={expPrefill} apiKey={store.apiKey}/>}
    {modal==="proxy"&&<AddProxySheet friends={friends} cards={cards} allCurrs={allCurrs} customPays={store.customPays} trip={trip} onSave={p=>store.addProxy(trip.id,p)} onClose={()=>setModal(null)}/>}
    ...
    {editingExp&&<EditExpenseSheet expense={editingExp} trip={trip} friends={friends} cards={cards} allCurrs={allCurrs} customPays={store.customPays} onSave={(patch,recs)=>{store.editExpense(trip.id,editingExp.id,patch,recs);setEditingExp(null);}} onClose={()=>setEditingExp(null)}/>}
    {buyingWish&&<BuyWishSheet wish={buyingWish} trip={trip} cards={cards} allCurrs={allCurrs} customPays={store.customPays} onBuy={(price,qty,currency,payment,cardId)=>{store.buyWish(trip.id,buyingWish.id,price,qty,currency,payment,cardId,cards);setBuyingWish(null);}} onClose={()=>setBuyingWish(null)}/>}
    {buyingSelf&&<BuySelfWishSheet wish={buyingSelf} trip={trip} cards={cards} allCurrs={allCurrs} customPays={store.customPays} onBuy={(price,qty,currency,payment,cardId)=>{store.buySelfWish(trip.id,buyingSelf.id,price,qty,currency,payment,cardId,cards);setBuyingSelf(null);}} onClose={()=>setBuyingSelf(null)}/>}
    {billFriend&&<BillSheet friend={billFriend} trip={trip} banks={banks} allCurrs={allCurrs} onClose={()=>setBillFriend(null)}/>}
    {printMode&&<PrintReport trip={trip} mode={printMode} allCurrs={allCurrs} cards={cards} onClose={()=>setPrintMode(null)}/>}
    <Toast data={toast}/>
  </div>);
}
function LedgerTab({trip,allCurrs,cards,customPays,onAdd,onAddWithScan,onDel,onEdit,apiKey,preferCurrs}){
  ...
  const handleScanFile=async e=>{
    ...
    catch(ex){setScanErr(`辨識失敗：${ex.message||"請重試"}`);}
    setScanning(false);e.target.value="";
  };
  const getPayLabel=(e)=>{
    if(e.payment==="cash")return"💵 現金";
    if(e.payment==="card"){const cn=e.cardName||(e.cardId?(cards.find(c=>c.id===e.cardId)?.name??""):"");return cn?`💳 ${cn}`:"💳 信用卡";}
    const cp=customPays?.find(p=>p.id===e.payment);if(cp)return`${cp.e} ${cp.name}`;
    if(e.payName)return e.payName;return e.payment;
  };
  return(<div style={{padding:"16px 16px 100px"}}>
    {!trip.archived&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
      ...
      <label style={{cursor:"pointer"}}>
        <div style={{padding:"14px",borderRadius:20,border:"2px dashed #B5A8C8",background:scanning?"#F0EDF8":"#EAE6F2",color:"#B5A8C8",fontSize:13,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          {scanning?<><span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>⟳</span>辨識中...</>:<><span style={{fontSize:18}}>📷</span>拍收據</>}
        </div>
    ...
    <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    {dates.length===0?<div style={{textAlign:"center",padding:"50px 0",color:"#BBBBA8"}}><div style={{fontSize:44,marginBottom:10}}>📒</div><div style={{fontFamily:SERIF,fontSize:15}}>還沒有支出記錄</div></div>
    :dates.map(date=>{const items=grouped[date];const dayMine=items.reduce((s,e)=>s+(e.myShare??0),0);
      return(<div key={date} style={{marginBottom:20}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {items.map(e=>{const cat=CATS.find(c=>c.id===e.category)||CATS[CATS.length-1];const currSym=allCurrs.find(c=>c.code===e.currency)?.sym??"";const platLabel=e.platformLabel||(e.platform&&e.platform!=="manual"?(PLATFORMS.find(p=>p.id===e.platform)?.l??""):"");const payLabel=getPayLabel(e);
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",gap:5,marginTop:3,flexWrap:"wrap",alignItems:"center"}}>
                    <span style={{fontSize:10,background:"#F5F3EF",color:"#8A8A7A",padding:"1px 7px",borderRadius:99}}>{payLabel}</span>
      ...
      }function CollectTab({trip,friends,allCurrs,onFriendsChange,onMarkManyPaid,onUnmarkPaid,onDelRec,onConvert,onBill}){
  const recs=trip.receivables??[];const pending=recs.filter(r=>!r.paid);const done=recs.filter(r=>r.paid);
  const pendTotal=pending.reduce((s,r)=>s+r.amount,0);const allF=[...new Set(pending.map(r=>r.friend))];
  const [editMembers,setEditMembers]=useState(false);const [newName,setNewName]=useState("");
  const [checked,setChecked]=useState({});
  const [showRepaySheet,setShowRepaySheet]=useState(false);
  const [repayType,setRepayType]=useState("transfer");
  const [repayCurrency,setRepayCurrency]=useState((trip.preferCurrs??["TWD"])[0]??"TWD");
  const [repayAmtInput,setRepayAmtInput]=useState("");
  const [repayManualCurr,setRepayManualCurr]=useState(false);
  ...
  const handleRepayConfirm=()=>{
    ...
    const amt=repayType==="cash"?+repayAmtInput:0;
    onMarkManyPaid(checkedIds,repayType,repayCurrency,amt);
    setChecked({});setShowRepaySheet(false);setRepayAmtInput("");setRepayManualCurr(false);
  };
  return(<div style={{padding:"16px 16px 100px"}}>
    ...
    {checkedIds.length>0&&<div style={{background:"#FFFFFF",borderRadius:16,border:"1.5px solid #88B098",padding:"12px 16px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:showRepaySheet?12:0}}>
        ...
        <Btn sm color="#88B098" onClick={()=>setShowRepaySheet(p=>!p)}>✓ 還款設定</Btn>
      </div>
      {showRepaySheet&&<div style={{borderTop:"1px solid #EDEBE6",paddingTop:12}}>
        <Lbl ch="還款方式"/>
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <button onClick={()=>setRepayType("transfer")} style={{flex:1,padding:"10px",borderRadius:12,border:`1.5px solid ${repayType==="transfer"?"#88B098":"#EDEBE6"}`,background:repayType==="transfer"?"#E2EFE8":"transparent",color:repayType==="transfer"?"#88B098":"#8A8A7A",fontSize:13,fontWeight:repayType==="transfer"?700:400,cursor:"pointer",fontFamily:SANS}}>🏦 匯款</button>
          <button onClick={()=>setRepayType("cash")} style={{flex:1,padding:"10px",borderRadius:12,border:`1.5px solid ${repayType==="cash"?"#C8B89A":"#EDEBE6"}`,background:repayType==="cash"?"#EDE6D8":"transparent",color:repayType==="cash"?"#C8B89A":"#8A8A7A",fontSize:13,fontWeight:repayType==="cash"?700:400,cursor:"pointer",fontFamily:SANS}}>💵 歸還現金</button>
        </div>
        {repayType==="cash"&&<>
          <Lbl ch="歸還幣別"/>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}>
            {preferCurrs.map(code=><button key={code} onClick={()=>{setRepayCurrency(code);setRepayManualCurr(false);}} style={{padding:"6px 12px",borderRadius:99,border:`1.5px solid ${repayCurrency===code&&!repayManualCurr?"#C8B89A":"#EDEBE6"}`,background:repayCurrency===code&&!repayManualCurr?"#EDE6D8":"transparent",color:repayCurrency===code&&!repayManualCurr?"#C8B89A":"#8A8A7A",fontSize:12,cursor:"pointer",fontFamily:SANS}}>{code}</button>)}
            <button onClick={()=>setRepayManualCurr(true)} style={{padding:"6px 12px",borderRadius:99,border:`1.5px solid ${repayManualCurr?"#C8B89A":"#EDEBE6"}`,background:repayManualCurr?"#EDE6D8":"transparent",color:repayManualCurr?"#C8B89A":"#8A8A7A",fontSize:12,cursor:"pointer",fontFamily:SANS}}>✏️ 其他</button>
          </div>
          {repayManualCurr&&<Inp placeholder="幣別代碼 如 JPY" value={repayCurrency} onChange={e=>setRepayCurrency(e.target.value.toUpperCase())} style={{marginBottom:8}}/>}
          <Lbl ch="歸還金額（原幣）"/>
          <Inp type="number" placeholder="0" value={repayAmtInput} onChange={e=>setRepayAmtInput(e.target.value)} style={{marginBottom:8}}/>
          {repayAmtInput&&+repayAmtInput>0&&<div style={{fontSize:11,color:"#A8B5A2",marginBottom:8}}>此金額將加入現金錢包 {repayCurrency} 餘額</div>}
        </>}
        <div style={{display:"flex",gap:8}}>
          <Btn onClick={handleRepayConfirm} color="#88B098" style={{flex:1}}>✓ 確認還款</Btn>
          <Btn outline color="#BBBBA8" onClick={()=>{setShowRepaySheet(false);setRepayAmtInput("");setRepayManualCurr(false);}} style={{flex:1}}>取消</Btn>
        </div>
      </div>}
    </div>}
    ...
    {allF.map(fr=>{const items=pending.filter(r=>r.friend===fr);const fTotal=items.reduce((s,r)=>s+r.amount,0);const allChk=items.every(r=>checked[r.id]);
      return(<div key={fr} style={{marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:32,height:32,borderRadius:"50%",background:"#E2EFE8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#88B098"}}>{fr[0]}</div><span style={{fontWeight:700,color:"#5A5A5A",fontSize:15}}>{fr}</span><span style={{fontSize:12,color:"#88B098",fontWeight:600}}>NT${fmt(fTotal)}</span></div>
        ...
        {items.map(r=><div key={r.id} style={{background:"#FFFFFF",borderRadius:20,border:`1.5px solid ${checked[r.id]?"#88B098":"#EDEBE6"}`,padding:"11px 14px",marginBottom:7}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div onClick={()=>toggleCheck(r.id)} style={{width:22,height:22,borderRadius:6,border:`2px solid ${checked[r.id]?"#88B098":"#DDD9D0"}`,background:checked[r.id]?"#88B098":"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{checked[r.id]&&<span style={{color:"#fff",fontSize:12,fontWeight:700,lineHeight:1}}>✓</span>}</div>
    ...
    {done.length>0&&<div style={{marginTop:20}}>
      ...
      {done.map(r=><div key={r.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,color:"#BBBBA8",padding:"8px 0",borderBottom:"1px solid #EDEBE6"}}>
        <div><span>{r.friend}・{r.note}・NT${fmt(r.amount)}</span>{r.repayType==="cash"&&r.repaymentCurrency&&<span style={{marginLeft:6,fontSize:11,background:"#EDE6D8",color:"#C8B89A",padding:"1px 6px",borderRadius:99}}>💵 {r.repaymentCurrency}</span>}</div>
...
function CashTab({trip,allCurrs,onAddWallet,onUpdateWallet,onDelWallet,onAddExchange,onDelExchange}){
  const wallets=trip.cashWallets??[];const exchanges=trip.exchanges??[];
  const [subTab,setSubTab]=useState("wallet");
  const [showAddWallet,setShowAddWallet]=useState(false);
  const [showAddExchange,setShowAddExchange]=useState(false);
  const [newW,setNewW]=useState({currency:"TWD",amount:""});
  const [ex,setEx]=useState({date:todayStr(),fromCurrency:"TWD",fromAmount:"",toCurrency:"JPY",toAmount:"",note:""});
  const [editingWid,setEditingWid]=useState(null);const [editAmt,setEditAmt]=useState("");
  const preferCurrs=trip.preferCurrs??["TWD"];
  const totalTWD=wallets.reduce((s,w)=>{const r=allCurrs.find(c=>c.code===w.currency)?.r??1;return s+w.amount*r;},0);
  return(<div style={{padding:"16px 16px 100px"}}>
    <div style={{background:"linear-gradient(135deg,#EDE6D8,#E8E0D0)",borderRadius:20,padding:"18px",marginBottom:16}}>
      <div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:7}}>手上現金合計（換算台幣）</div>
      <div style={{fontSize:30,fontWeight:800,color:"#C8B89A",fontFamily:SERIF}}>NT${fmt(totalTWD)}</div>
      <div style={{fontSize:12,color:"#BBBBA8",marginTop:4}}>{wallets.length} 種幣別</div>
    </div>
    <div style={{display:"flex",gap:8,marginBottom:16}}>{[{id:"wallet",l:"💵 現金錢包"},{id:"exchange",l:"💱 換匯紀錄"}].map(t=><Pill key={t.id} active={subTab===t.id} color="#C8B89A" onClick={()=>setSubTab(t.id)}>{t.l}</Pill>)}</div>
    {subTab==="wallet"&&<>
      {!trip.archived&&<button onClick={()=>setShowAddWallet(p=>!p)} style={{width:"100%",padding:"13px",borderRadius:18,border:"2px dashed #C8B89A",background:"#EDE6D8",color:"#C8B89A",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SANS,marginBottom:12,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><span style={{fontSize:18}}>+</span>新增現金</button>}
      {showAddWallet&&<div style={{background:"#FFFFFF",borderRadius:18,border:"1px solid #EDEBE6",padding:"16px",marginBottom:14}}>
        <Lbl ch="幣別"/><CurrSel value={newW.currency} onChange={e=>setNewW(p=>({...p,currency:e.target.value}))} allCurrs={allCurrs} preferCurrs={preferCurrs}/><div style={{marginBottom:10}}/>
        <Lbl ch="攜帶金額"/><Inp type="number" placeholder="0" value={newW.amount} onChange={e=>setNewW(p=>({...p,amount:e.target.value}))} style={{marginBottom:12}}/>
        <div style={{display:"flex",gap:8}}>
          <Btn onClick={()=>{if(!newW.amount||+newW.amount<=0)return;onAddWallet({currency:newW.currency,amount:+newW.amount});setNewW({currency:"TWD",amount:""});setShowAddWallet(false);}} style={{flex:1}}>+ 新增</Btn>
          <Btn outline color="#BBBBA8" onClick={()=>setShowAddWallet(false)} style={{flex:1}}>取消</Btn>
        </div>
      </div>}
      {wallets.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:"#BBBBA8"}}><div style={{fontSize:36,marginBottom:8}}>💵</div><div style={{fontFamily:SERIF,fontSize:14}}>還沒有現金記錄</div><div style={{fontSize:12,marginTop:4}}>出發前登記帶多少現金</div></div>}
      {wallets.map(w=>{const curr=allCurrs.find(c=>c.code===w.currency);const twdVal=w.amount*(curr?.r??1);
        return(<div key={w.id} style={{background:"#FFFFFF",borderRadius:20,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:44,height:44,borderRadius:14,background:"#EDE6D8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:700,color:"#C8B89A",flexShrink:0}}>{curr?.sym??w.currency}</div>
            <div style={{flex:1}}>
              <div style={{fontSize:13,color:"#8A8A7A",marginBottom:2}}>{curr?.l??w.currency}</div>
              {editingWid===w.id
                ?<div style={{display:"flex",gap:6,marginTop:4}}><Inp type="number" value={editAmt} onChange={e=>setEditAmt(e.target.value)} style={{flex:1,padding:"8px 10px",fontSize:14}}/><button onClick={()=>{onUpdateWallet(w.id,+editAmt);setEditingWid(null);}} style={{padding:"8px 14px",borderRadius:10,border:"none",background:"#88B098",color:"#fff",fontSize:12,cursor:"pointer",fontFamily:SANS}}>✓</button><button onClick={()=>setEditingWid(null)} style={{padding:"8px 10px",borderRadius:10,border:"1px solid #EDEBE6",background:"transparent",color:"#BBBBA8",fontSize:12,cursor:"pointer",fontFamily:SANS}}>✕</button></div>
                :<div><div style={{fontSize:22,fontWeight:800,color:"#5A5A5A"}}>{fmtD(w.amount,0)} <span style={{fontSize:13,color:"#8A8A7A"}}>{w.currency}</span></div>{w.initialAmount!=null&&w.initialAmount!==w.amount&&<div style={{fontSize:11,color:"#BBBBA8",marginTop:2}}>原始攜帶：{fmtD(w.initialAmount,0)} {w.currency}</div>}</div>}
              <div style={{fontSize:12,color:"#BBBBA8",marginTop:2}}>≈ NT${fmt(twdVal)}</div>
            </div>
            {!trip.archived&&editingWid!==w.id&&<div style={{display:"flex",gap:6}}>
              <button onClick={()=>{setEditingWid(w.id);setEditAmt(String(w.amount));}} style={{padding:"7px 10px",borderRadius:9,border:"1px solid #EDEBE6",background:"transparent",color:"#8A8A7A",fontSize:11,cursor:"pointer",fontFamily:SANS}}>✏️</button>
              <button onClick={()=>{if(window.confirm(`刪除 ${w.currency} 現金？`))onDelWallet(w.id);}} style={{padding:"7px 10px",borderRadius:9,border:"1px solid #FAE8E3",background:"#FAE8E3",color:"#D4806A",fontSize:11,cursor:"pointer"}}>🗑</button>
            </div>}
          </div>
        </div>);
      })}
    ...
    {subTab==="exchange"&&<>
      {!trip.archived&&<button onClick={()=>setShowAddExchange(p=>!p)} style={{width:"100%",padding:"13px",borderRadius:18,border:"2px dashed #A8B5A2",background:"#E4EBE2",color:"#A8B5A2",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SANS,marginBottom:12,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><span style={{fontSize:18}}>+</span>新增換匯紀錄</button>}
      {showAddExchange&&<div style={{background:"#FFFFFF",borderRadius:18,border:"1px solid #EDEBE6",padding:"16px",marginBottom:14}}>
        <Lbl ch="日期"/><Inp type="date" value={ex.date} onChange={e=>setEx(p=>({...p,date:e.target.value}))} style={{marginBottom:10}}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:8,alignItems:"end",marginBottom:10}}>
          <div><Lbl ch="支出幣別"/><CurrSel value={ex.fromCurrency} onChange={e=>setEx(p=>({...p,fromCurrency:e.target.value}))} allCurrs={allCurrs} preferCurrs={preferCurrs}/><Inp type="number" placeholder="金額" value={ex.fromAmount} onChange={e=>setEx(p=>({...p,fromAmount:e.target.value}))} style={{marginTop:6}}/></div>
          <div style={{textAlign:"center",fontSize:20,paddingBottom:8,color:"#A8B5A2"}}>→</div>
          <div><Lbl ch="換得幣別"/><CurrSel value={ex.toCurrency} onChange={e=>setEx(p=>({...p,toCurrency:e.target.value}))} allCurrs={allCurrs} preferCurrs={preferCurrs}/><Inp type="number" placeholder="換得金額" value={ex.toAmount} onChange={e=>setEx(p=>({...p,toAmount:e.target.value}))} style={{marginTop:6}}/></div>
        </div>
        {ex.fromAmount&&ex.toAmount&&+ex.fromAmount>0&&+ex.toAmount>0&&<div style={{background:"#E4EBE2",borderRadius:12,padding:"8px 12px",fontSize:12,color:"#A8B5A2",marginBottom:10}}>匯率：1 {ex.fromCurrency} = {(+ex.toAmount/+ex.fromAmount).toFixed(4)} {ex.toCurrency}</div>}
        <Lbl ch="備註（換匯地點⋯）"/><Inp placeholder="機場換匯、銀行⋯" value={ex.note} onChange={e=>setEx(p=>({...p,note:e.target.value}))} style={{marginBottom:12}}/>
        <div style={{display:"flex",gap:8}}>
          <Btn onClick={()=>{if(!ex.fromAmount||!ex.toAmount||+ex.fromAmount<=0||+ex.toAmount<=0)return;onAddExchange({...ex,fromAmount:+ex.fromAmount,toAmount:+ex.toAmount});setEx({date:todayStr(),fromCurrency:"TWD",fromAmount:"",toCurrency:"JPY",toAmount:"",note:""});setShowAddExchange(false);}} color="#A8B5A2" style={{flex:1}}>記錄換匯</Btn>
          <Btn outline color="#BBBBA8" onClick={()=>setShowAddExchange(false)} style={{flex:1}}>取消</Btn>
        </div>
      </div>}
      {exchanges.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:"#BBBBA8"}}><div style={{fontSize:36,marginBottom:8}}>💱</div><div style={{fontFamily:SERIF,fontSize:14}}>還沒有換匯紀錄</div><div style={{fontSize:12,marginTop:4}}>刪除換匯紀錄會自動還原現金金額</div></div>}
      {exchanges.map(ex=>{const rate=(+ex.toAmount/+ex.fromAmount).toFixed(4);
        return(<div key={ex.id} style={{background:"#FFFFFF",borderRadius:20,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#D4806A"}}>{fmtD(ex.fromAmount,0)} {ex.fromCurrency}</span><span style={{color:"#A8B5A2",fontSize:14}}>→</span><span style={{fontSize:14,fontWeight:700,color:"#88B098"}}>{fmtD(ex.toAmount,0)} {ex.toCurrency}</span></div>
              <div style={{fontSize:11,color:"#BBBBA8"}}>匯率 1 {ex.fromCurrency} = {rate} {ex.toCurrency}・{ex.date}</div>
              {ex.note&&<div style={{fontSize:11,color:"#8A8A7A",marginTop:2}}>{ex.note}</div>}
            </div>
            {!trip.archived&&<button onClick={()=>{if(window.confirm("確定刪除此換匯紀錄？現金金額將自動還原。"))onDelExchange(ex.id);}} style={{padding:"7px 10px",borderRadius:9,border:"1px solid #FAE8E3",background:"#FAE8E3",color:"#D4806A",fontSize:11,cursor:"pointer"}}>🗑</button>}
          </div>
        </div>);
      })}
    </>}
...
function ProxyTab({trip,allCurrs,onAddWish,onBuyWish,onDelWish,onAddSelfWish,onBuySelfWish,onDelSelfWish,onAdd,onDel,onMarkManyProxyPaid,onToggle,onBill}){
  ...
  const wishlist=trip.wishlist??[];const selflist=trip.selflist??[];
  const pendingWish=wishlist.filter(w=>!w.done);const pendingSelf=selflist.filter(w=>!w.done);
  const proxies=trip.proxies??[];const unpaid=proxies.filter(p=>!p.paid);const paid=proxies.filter(p=>p.paid);
  const pendTotal=unpaid.reduce((s,p)=>s+(p.totalTWD??0),0);const allB=[...new Set(unpaid.map(p=>p.buyer))];
  const [checked,setChecked]=useState({});
  const toggleCheck=id=>setChecked(p=>({...p,[id]:!p[id]}));
  const checkedIds=Object.keys(checked).filter(id=>checked[id]);
  ...
  return(<div style={{padding:"16px 16px 100px"}}>
      ...
      {pendingWish.map(w=><div key={w.id} style={{background:"#FFFFFF",borderRadius:20,border:"1px solid #EDEBE6",padding:"14px",marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            {!trip.archived&&<button onClick={()=>{if(window.confirm(`確定刪除「${w.item}」？`))onDelWish(w.id);}} style={{padding:"7px 12px",borderRadius:10,border:"1px solid #FAE8E3",background:"#FAE8E3",color:"#D4806A",fontSize:12,cursor:"pointer",fontFamily:SANS}}>🗑</button>}
          </div>
        </div>
      </div>)}
    ...
    {view==="self"&&<>
      ...
      {pendingSelf.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:"#BBBBA8"}}><div style={{fontSize:36,marginBottom:8}}>🛒</div><div style={{fontFamily:SERIF,fontSize:14}}>自用清單是空的</div></div>}
      {pendingSelf.map(w=><div key={w.id} style={{background:"#FFFFFF",borderRadius:20,border:"1px solid #EDEBE6",padding:"14px",marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            {!trip.archived&&<button onClick={()=>{if(window.confirm(`確定刪除「${w.item}」？`))onDelSelfWish(w.id);}} style={{padding:"7px 12px",borderRadius:10,border:"1px solid #FAE8E3",background:"#FAE8E3",color:"#D4806A",fontSize:12,cursor:"pointer",fontFamily:SANS}}>🗑</button>}
          </div>
        </div>
      </div>)}
    ...
    {view==="done"&&<>
      ...
      {allB.map(buyer=>{const items=unpaid.filter(p=>p.buyer===buyer);const bTotal=items.reduce((s,p)=>s+(p.totalTWD??0),0);const allChk=items.every(p=>checked[p.id]);
        return(<div key={buyer} style={{marginBottom:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:32,height:32,borderRadius:"50%",background:"#EAE6F2",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#B5A8C8"}}>{buyer[0]}</div><span style={{fontWeight:700,color:"#5A5A5A",fontSize:15}}>{buyer}</span><span style={{fontSize:12,color:"#B5A8C8",fontWeight:600}}>NT${fmt(bTotal)}</span></div>
            <div style={{display:"flex",gap:6}}>
          ...
          {items.map(p=>{const sym=allCurrs.find(c=>c.code===p.currency)?.sym??"";
            return(<div key={p.id} style={{background:"#FFFFFF",borderRadius:20,border:`1.5px solid ${checked[p.id]?"#B5A8C8":"#EDEBE6"}`,padding:"12px 14px",marginBottom:7}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div onClick={()=>toggleCheck(p.id)} style={{width:22,height:22,borderRadius:6,border:`2px solid ${checked[p.id]?"#B5A8C8":"#DDD9D0"}`,background:checked[p.id]?"#B5A8C8":"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{checked[p.id]&&<span style={{color:"#fff",fontSize:12,fontWeight:700,lineHeight:1}}>✓</span>}</div>
      ...
      {paid.length>0&&<div style={{marginTop:16}}><div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:10}}>已付款紀錄</div>{paid.map(p=><div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,color:"#BBBBA8",padding:"8px 0",borderBottom:"1px solid #EDEBE6"}}><span>{p.buyer}・{p.item}・NT${fmt(p.totalTWD??0)}</span><button onClick={()=>onToggle(p.id)} style={{padding:"4px 8px",borderRadius:8,border:"1px solid #EDEBE6",background:"transparent",color:"#BBBBA8",fontSize:10,cursor:"pointer",fontFamily:SANS,flexShrink:0,marginLeft:8}}>↩ 取消</button></div>)}</div>}
...
function StatsTab({trip,allCurrs,cards,customPays,onOpenPrint,onExportCSV,onSetCustomDays}){
  const myTotal=trip.expenses.reduce((s,e)=>s+(e.myShare??0),0);
  ...
  const byCard=cards.map(c=>({...c,sum:trip.expenses.filter(e=>e.cardId===c.id).reduce((s,e)=>s+(e.myShare??0),0)})).filter(c=>c.sum>0).sort((a,b)=>b.sum-a.sum);
  const wallets=trip.cashWallets??[];
  const cashTWD=wallets.reduce((s,w)=>s+w.amount*(allCurrs.find(c=>c.code===w.currency)?.r??1),0);
  const autodays=[...new Set(trip.expenses.map(e=>e.date))].length;
  const days=trip.customDays??autodays;
  ...
  return(<div style={{padding:"16px 16px 100px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4,flexWrap:"wrap"}}>
        {editDays
          ?<div style={{display:"flex",gap:6,alignItems:"center"}}>
            <Inp type="number" value={daysInput} onChange={e=>setDaysInput(e.target.value)} style={{width:70,padding:"4px 8px",fontSize:13}}/>
            <span style={{fontSize:12,color:"#BBBBA8"}}>天</span>
            <button onClick={()=>{const d=+daysInput;if(d>0)onSetCustomDays(d);setEditDays(false);}} style={{padding:"4px 10px",borderRadius:8,border:"none",background:"#88B098",color:"#fff",fontSize:11,cursor:"pointer",fontFamily:SANS}}>✓</button>
            <button onClick={()=>setEditDays(false)} style={{padding:"4px 8px",borderRadius:8,border:"1px solid #EDEBE6",background:"transparent",color:"#BBBBA8",fontSize:11,cursor:"pointer"}}>✕</button>
          </div>
          :<div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:12,color:"#BBBBA8"}}>{trip.expenses.length}筆・{days}天{trip.customDays?"（自訂）":"（自動）"}</span>
            <button onClick={()=>{setDaysInput(String(days));setEditDays(true);}} style={{padding:"2px 8px",borderRadius:7,border:"1px solid #EDEBE6",background:"transparent",color:"#BBBBA8",fontSize:10,cursor:"pointer",fontFamily:SANS}}>✏️ 修改</button>
          </div>}
      </div>
    </div>
    ...
    {days>0&&<div style={{background:"#EDE6D8",borderRadius:16,padding:"12px 16px",marginBottom:12,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:12,color:"#8A7040"}}>日均花費</span><span style={{fontSize:15,fontWeight:700,color:"#C8B89A"}}>NT${fmt(myTotal/days)}</span></div>}
    {wallets.length>0&&<div style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:12}}>
      <div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:10}}>💵 手上現金餘額</div>
      {wallets.map(w=>{const curr=allCurrs.find(c=>c.code===w.currency);const twdVal=w.amount*(curr?.r??1);return(<div key={w.id} style={{marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:"#5A5A5A"}}>{curr?.sym} {fmtD(w.amount,0)} {w.currency}</span><span style={{fontSize:12,color:"#8A8A7A"}}>≈ NT${fmt(twdVal)}</span></div>{w.initialAmount!=null&&w.initialAmount!==w.amount&&<div style={{fontSize:10,color:"#BBBBA8"}}>原始攜帶：{fmtD(w.initialAmount,0)} {w.currency}</div>}</div>);})}
      <div style={{borderTop:"1px solid #EDEBE6",paddingTop:8,marginTop:4,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:12,color:"#BBBBA8"}}>合計</span><span style={{fontSize:14,fontWeight:700,color:"#C8B89A"}}>NT${fmt(cashTWD)}</span></div>
    </div>}
    {byCard.length>0&&<div style={{background:"#FFFFFF",borderRadius:16,border:"1px solid #EDEBE6",padding:"14px 16px",marginBottom:12}}>
      <div style={{fontSize:10,color:"#BBBBA8",letterSpacing:1.8,fontWeight:600,marginBottom:10}}>💳 信用卡消費</div>
      {byCard.map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,color:"#5A5A5A"}}>💳 {c.name}</span><span style={{fontSize:13,fontWeight:600,color:"#C8B89A"}}>NT${fmt(c.sum)}</span></div>)}
    </div>}
    ...
    <Btn onClick={onExportCSV} color="#A8B5A2" outline style={{width:"100%"}}>📊 匯出完整 CSV</Btn>
  </div>);
}
function PrintReport({trip,mode,allCurrs,cards,onClose}){
  const myTotal=trip.expenses.reduce((s,e)=>s+(e.myShare??0),0);
  ...
  const unpaidProxies=(trip.proxies??[]).filter(p=>!p.paid);
  const wallets=trip.cashWallets??[];
  const exchanges=trip.exchanges??[];
  const cashTWD=wallets.reduce((s,w)=>s+w.amount*(allCurrs.find(c=>c.code===w.currency)?.r??1),0);

  const doPrint=()=>{
    const content=document.getElementById("tf-print-area").innerHTML;
    const printWin=window.open("","_blank","width=800,height=600");
    if(!printWin)return;
    printWin.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${trip.name} 旅行報表</title><style>
      *{box-sizing:border-box;margin:0;padding:0;}
      body{font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;background:#fff;color:#5A5A5A;}
      @media print{
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(60,50,40,0.5)",backdropFilter:"blur(10px)",zIndex:500,overflowY:"auto"}}>
    <div style={{position:"sticky",top:0,zIndex:10,background:"rgba(249,248,246,0.96)",backdropFilter:"blur(12px)",borderBottom:"1px solid #EDEBE6",padding:"12px 16px",display:"flex",gap:10,alignItems:"center",maxWidth:680,margin:"0 auto"}}>
      ...
      <button onClick={doPrint} style={{background:"#C9A89A",border:"none",borderRadius:12,padding:"10px 20px",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:SANS}}>🖨️ 列印 / PDF</button>
    </div>
    <div id="tf-print-area" style={{maxWidth:680,margin:"0 auto",padding:"0 0 60px",background:"#fff"}}>
      ...
      <div style={{padding:"28px 36px 0"}}>
        ...
        {mode==="detail"&&<>
          <div style={{fontSize:12,fontWeight:700,color:"#C9A89A",marginBottom:16,paddingBottom:6,borderBottom:"2px solid #F0E6E1"}}>📖 完整支出明細（共 {trip.expenses.length} 筆）</div>
          {dates.map(date=>{const items=grouped[date];const dayMine=items.reduce((s,e)=>s+(e.myShare??0),0);return(<div key={date} style={{marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",paddingBottom:8,borderBottom:"1px solid #F0E6E1",marginBottom:8}}><span style={{fontSize:12,fontWeight:700,color:"#5A5A5A"}}>{date.replace(/-/g," / ")}</span><span style={{fontSize:12,color:"#C9A89A",fontWeight:700}}>NT${fmt(dayMine)}</span></div>{items.map(e=>{const cat=CATS.find(c=>c.id===e.category)||CATS[CATS.length-1];const sym=allCurrs.find(c=>c.code===e.currency)?.sym??"";const cardName=e.cardName||(e.cardId?(cards.find(c=>c.id===e.cardId)?.name??""):"");const platLabel=e.platformLabel||(e.platform&&e.platform!=="manual"?(PLATFORMS.find(p=>p.id===e.platform)?.l??""):"");return(<div key={e.id} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"8px 10px",marginBottom:4,background:"#F9F8F6",borderRadius:10}}><span style={{fontSize:18,flexShrink:0}}>{cat.e}</span><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#5A5A5A"}}>{e.note||cat.l}{platLabel?` · ${platLabel}`:""}</div>{e.detail&&<div style={{fontSize:10,color:"#8A8A7A"}}>{e.detail}</div>}{cardName&&<div style={{fontSize:10,color:"#C8B89A"}}>💳 {cardName}</div>}{e.currency!=="TWD"&&<div style={{fontSize:10,color:"#BBBBA8"}}>{sym}{fmt(e.amount)}</div>}</div><div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:13,fontWeight:700,color:"#C9A89A"}}>NT${fmt(e.myShare??0)}</div>{e.isOverseas===false&&<div style={{fontSize:9,color:"#C9A89A"}}>🇹🇼</div>}</div></div>);})}</div>);})}
        </>}
        {unpaidProxies.length>0&&<div style={{marginTop:24}}>
          <div style={{fontSize:12,fontWeight:700,color:"#B5A8C8",marginBottom:12,paddingBottom:6,borderBottom:"2px solid #EAE6F2"}}>🛍️ 代購待收款</div>
          {[...new Set(unpaidProxies.map(p=>p.buyer))].map(buyer=>{const items=unpaidProxies.filter(p=>p.buyer===buyer);const bTotal=items.reduce((s,p)=>s+(p.totalTWD??0),0);return(<div key={buyer} style={{marginBottom:16,background:"#F9F6FF",borderRadius:14,padding:"12px 14px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:700,color:"#5A5A5A"}}>👤 {buyer}</span><span style={{fontSize:14,fontWeight:800,color:"#B5A8C8"}}>NT${fmt(bTotal)}</span></div>{mode==="detail"&&items.map(p=>{const curr=allCurrs.find(c=>c.code===p.currency);const sym=curr?.sym??p.currency;return(<div key={p.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#8A8A7A",marginBottom:4,paddingLeft:8}}><span>{p.item} ×{p.qty}　{sym}{fmtD(p.price,0)}</span><span>NT${fmt(p.totalTWD??0)}</span></div>);})}</div>);})}
        </div>}
        {allFriends.length>0&&<div style={{marginTop:24}}>
          <div style={{fontSize:12,fontWeight:700,color:"#88B098",marginBottom:12,paddingBottom:6,borderBottom:"2px solid #E2EFE8"}}>💸 代墊待收款</div>
          {allFriends.map(f=>{const items=(trip.receivables??[]).filter(r=>r.friend===f&&!r.paid);if(!items.length)return null;const fTotal=items.reduce((s,r)=>s+r.amount,0);return(<div key={f} style={{marginBottom:16,background:"#F0FAF4",borderRadius:14,padding:"12px 14px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:mode==="detail"?8:0}}><span style={{fontWeight:700,color:"#5A5A5A",fontSize:13}}>👤 {f}</span><span style={{fontWeight:800,color:"#88B098",fontSize:14}}>NT${fmt(fTotal)}</span></div>{mode==="detail"&&items.map(r=><div key={r.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#8A8A7A",marginBottom:3,paddingLeft:8}}><span>{r.note}（{r.date}）</span><span>NT${fmt(r.amount)}</span></div>)}</div>);})}
        </div>}
        {wallets.length>0&&<div style={{marginTop:24}}>
          <div style={{fontSize:12,fontWeight:700,color:"#C8B89A",marginBottom:12,paddingBottom:6,borderBottom:"2px solid #EDE6D8"}}>💵 現金錢包餘額</div>
          <div style={{background:"#FDFAF5",borderRadius:14,padding:"12px 14px"}}>
            {wallets.map(w=>{const curr=allCurrs.find(c=>c.code===w.currency);const twdVal=w.amount*(curr?.r??1);return(<div key={w.id} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:13,color:"#5A5A5A"}}>{curr?.sym} {fmtD(w.amount,0)} {w.currency}</span><span style={{fontSize:12,color:"#8A8A7A"}}>≈ NT${fmt(twdVal)}</span></div>{w.initialAmount!=null&&w.initialAmount!==w.amount&&<div style={{fontSize:10,color:"#BBBBA8",paddingLeft:4}}>原始攜帶：{fmtD(w.initialAmount,0)} {w.currency}</div>}</div>);})}
            <div style={{borderTop:"1px solid #EDEBE6",paddingTop:8,marginTop:4,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:12,color:"#BBBBA8"}}>合計</span><span style={{fontSize:14,fontWeight:700,color:"#C8B89A"}}>NT${fmt(cashTWD)}</span></div>
          </div>
        </div>}
        {mode==="detail"&&exchanges.length>0&&<div style={{marginTop:24}}>
          <div style={{fontSize:12,fontWeight:700,color:"#A8B5A2",marginBottom:12,paddingBottom:6,borderBottom:"2px solid #E4EBE2"}}>💱 換匯紀錄</div>
          {exchanges.map(ex=>{const rate=(+ex.toAmount/+ex.fromAmount).toFixed(4);return(<div key={ex.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,color:"#5A5A5A",padding:"7px 10px",marginBottom:4,background:"#F5F8F5",borderRadius:10}}><div><span style={{color:"#D4806A",fontWeight:600}}>{fmtD(ex.fromAmount,0)} {ex.fromCurrency}</span><span style={{color:"#A8B5A2",margin:"0 6px"}}>→</span><span style={{color:"#88B098",fontWeight:600}}>{fmtD(ex.toAmount,0)} {ex.toCurrency}</span>{ex.note&&<span style={{color:"#BBBBA8",marginLeft:6}}>{ex.note}</span>}</div><div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:10,color:"#BBBBA8"}}>{ex.date}</div><div style={{fontSize:10,color:"#BBBBA8"}}>1:{rate}</div></div></div>);})}
        </div>}
...
export default function App(){
  ...
  return(<div style={{minHeight:"100vh",background:"#F9F8F6",fontFamily:SANS,maxWidth:430,margin:"0 auto"}}>
    <div style={{height:44}}/>
    {screen==="list"&&<>
      <TripList trips={store.trips} activeId={store.activeId} onSelect={openTrip} onCreate={()=>setModal("new")} onArchive={store.archiveTrip} onReopen={store.reopenTrip} onDelete={store.deleteTrip} onSettings={()=>setModal("settings")} allCurrs={store.allCurrs}/>
      {modal==="new"&&<NewTripSheet onSave={d=>{const id=store.createTrip(d);openTrip(id);}} onClose={()=>setModal(null)} allCurrs={store.allCurrs}/>}
      {modal==="settings"&&<SettingsSheet banks={store.banks} setBanks={store.setBanks} cards={store.cards} setCards={store.setCards} friends={store.friends} setFriends={store.setFriends} trips={store.trips} customCurrs={store.customCurrs} setCustomCurrs={store.setCustomCurrs} customPays={store.customPays} setCustomPays={store.setCustomPays} apiKey={store.apiKey} setApiKey={store.setApiKey} onRestore={store.restoreAll} onClose={()=>setModal(null)}/>}
    </>}
